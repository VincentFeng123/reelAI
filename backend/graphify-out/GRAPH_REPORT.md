...
## Corpus Check
- 332 files · ~503,478 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6751 nodes · 19073 edges · 157 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 6594 edges (avg confidence: 0.5)
...
1. `Sentence` - 283 edges
2. `ReelService` - 199 edges
3. `GenerationContext` - 184 edges
4. `Unit` - 180 edges
5. `CancellationError` - 150 edges
6. `DatabaseIntegrityError` - 138 edges
7. `ProviderError` - 135 edges
8. `YouTubeService` - 132 edges
9. `RateLimitedError` - 105 edges
10. `MediumRegressionTests` - 104 edges

## Surprising Connections (you probably didn't know these)
- `Corruption-based judge probe (LLMBar pattern, arXiv:2310.07641) — measure per-ga` --uses--> `JudgeVerdict`  [INFERRED]
  eval/judge_probe.py → pipeline/assemble/validate.py
- `Clamped inclusive sentence range of a clip spec (mirrors metrics._clip_text).` --uses--> `JudgeVerdict`  [INFERRED]
  eval/judge_probe.py → pipeline/assemble/validate.py
- `Remove the clip's first sentence — a mid-idea open. None for 1-sentence clips.` --uses--> `JudgeVerdict`  [INFERRED]
  eval/judge_probe.py → pipeline/assemble/validate.py
- `Remove the clip's last sentence — a missing conclusion. None for 1-sentence clip` --uses--> `JudgeVerdict`  [INFERRED]
  eval/judge_probe.py → pipeline/assemble/validate.py
- `Remove the sentences of an in-clip unit that an in-clip Reference points back at` --uses--> `JudgeVerdict`  [INFERRED]
  eval/judge_probe.py → pipeline/assemble/validate.py
...
Cohesion: 0.01
Nodes (458): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+450 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (360): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 3-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Use a savepoint so multi-row assessment writes cannot be half-persisted., cue_text() (+352 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (341): Enum, _acquire_selector_slot(), _apply_enrichment(), _Assessment, _AssessmentDraft, _AtomicDeclarative, _authoritative_pro(), _boundary_prompts() (+333 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (79): Exception, APIStatusError, BadRequestError, chat(), get_client(), parse_groq_duration(), RateBudget, RateLimitError (+71 more)

### Community 4 - "Community 4"
Cohesion: 0.02
Nodes (222): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed(), _bare_video_id() (+214 more)
...
Cohesion: 0.02
Nodes (134): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+126 more)
...
Cohesion: 0.22
Nodes (123): AssessmentCancelledError, AssessmentService, BaseModel, EngineError, Base error for the clip engine., GenerationQueueFullError, JobLeaseLostError, The worker no longer owns a live lease for the requested write. (+115 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (117): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+109 more)
...
Cohesion: 0.03
Nodes (116): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+108 more)

### Community 13 - "Community 13"
Cohesion: 0.03
Nodes (104): _compact_plan(), _intent_plan(), _proposal(), test_articleless_look_at_visual_noun_remains_visual_dependent(), test_assignment_leadin_is_retained_when_its_cue_contains_grounded_teaching(), test_back_to_navigation_does_not_delete_worked_example_setup(), test_bare_look_at_this_remains_visual_dependent(), test_boundary_quote_reanchoring_never_discards_substantive_context() (+96 more)
...
Cohesion: 0.04
Nodes (108): _apply_verdict(), _cut_segment(), EdgeVerdict, _probe_clip(), VID2 — edge probe (Tier 1 video judge): advisory audio-boundary check on shipped, Advisory edge probe over the shipped survivors — mutates each spec's warnings in, Cut a small [start_s, start_s+dur_s] mp4 segment via ffmpeg (cut.build_cmd) into, Cut the head + tail, send ONE inline video judge call, return the parsed EdgeVer (+100 more)
...
Cohesion: 0.07
Nodes (64): _discovery(), _one_cue_selector_result(), _pipeline(), _pipeline_with_semantic(), _plan(), _quality_clip(), test_acoustic_boundary_plan_fails_closed_for_missing_cue_ids(), test_acoustic_result_crossing_prior_speech_is_stored_but_deferred() (+56 more)

### Community 20 - "Community 20"
Cohesion: 0.08
Nodes (76): _cue(), _proposal(), _report(), test_agenda_recovery_keeps_a_substantive_setup_before_later_work(), test_agenda_recovery_keeps_same_cue_and_question_setups(), test_ambiguous_evidence_cannot_span_a_topic_reset(), test_anaphoric_show_continuation_keeps_the_reasoning_after_a_result(), test_compact_comparison_uses_relationship_evidence_and_stops_at_third_unit() (+68 more)
...
Cohesion: 0.05
Nodes (37): _age_seconds(), _canonical_json(), MemoryProviderCache, normalize_filters(), normalize_language(), normalize_query(), _parse_time(), Versioned DB caches for Supadata search evidence and timed transcript artifacts. (+29 more)
...
Cohesion: 0.13
Nodes (36): _complete(), _seed_completed_accuracy(), _seed_pending_session(), _seed_promotion_outcome(), _seed_reel(), test_answer_rejects_an_unrepairable_partial_without_storing_an_attempt(), test_answer_repairs_legacy_partial_before_accepting_a_cached_question(), test_answer_reveals_key_then_applies_concept_outcomes_once() (+28 more)
...
Cohesion: 0.11
Nodes (19): _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows(), _ensure_learner_progress(), _information_units() (+11 more)
...
## Knowledge Gaps
- **358 isolated node(s):** `YouTube embed URL that plays only [start, end] (whole seconds: floor start / cei`, `The universal content-role ontology (spec §4).  Universal roles are a stable, cl`, `Map an arbitrary LLM-emitted role string onto a valid universal role.      Unkno`, `A strict 'return only JSON of this shape' instruction for the Groq json_object p`, `Best-effort OpenAI-compatible json_schema wrapper around a Pydantic model.` (+353 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 144`** (2 nodes): `test_topics_types.py`, `test_schemas_and_dataclasses_exist()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (2 nodes): `test_youtube_impersonation_target_is_supported()`, `test_youtube_transport.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 146`** (2 nodes): `test_sqlite_provider_cache_round_trip_and_tombstone_filter()`, `test_provider_cache_db.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 147`** (2 nodes): `test_singleflight_coalesces_identical_concurrent_work()`, `test_singleflight.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 154`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `Sentence` connect `Community 2` to `Community 0`, `Community 97`, `Community 1`, `Community 5`, `Community 6`, `Community 107`, `Community 111`, `Community 87`, `Community 25`, `Community 28`, `Community 93`, `Community 94`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 3` to `Community 1`, `Community 8`, `Community 9`, `Community 41`, `Community 84`, `Community 62`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `GenerationContext` connect `Community 1` to `Community 33`, `Community 3`, `Community 9`, `Community 24`, `Community 26`, `Community 30`, `Community 31`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Are the 277 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 277 INFERRED edges - model-reasoned connections that need verification._
- **Are the 82 inferred relationships involving `ReelService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ReelService` has 82 INFERRED edges - model-reasoned connections that need verification._
- **Are the 165 inferred relationships involving `GenerationContext` (e.g. with `_HTTPStatusError` and `_ModelSequence`) actually correct?**
  _`GenerationContext` has 165 INFERRED edges - model-reasoned connections that need verification._
