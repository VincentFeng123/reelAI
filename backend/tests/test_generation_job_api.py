...
def test_fresh_inventory_and_selector_cache_share_current_contract() -> None:
    ...
    assert generation_jobs.REQUEST_SCHEMA_VERSION == "adaptive_clip_concepts_v7"
    assert segment_cache.SELECTION_CONTRACT_VERSION == "quality_silence_v41"
    ...
    assert "quality_silence_v40" in ReelService.DIFFICULTY_FALLBACK_CONTRACTS
    assert "quality_silence_v41" in ReelService.DIFFICULTY_FALLBACK_CONTRACTS


def test_current_request_filter_rejects_stale_or_unversioned_inventory() -> None:
    current = {"reel_id": "current", "selection_contract_version": "quality_silence_v41"}
    internal_current = {
        "reel_id": "internal-current",
        "_selection_contract_version": "quality_silence_v41",
    }
    ...
    assert main._current_selection_contract_reels([
        ...
        {"reel_id": "previous-selector", "selection_contract_version": "quality_silence_v39"},
        {"reel_id": "pre-cost-guard", "selection_contract_version": "quality_silence_v40"},
        {"reel_id": "missing"},
        current,
        internal_current,
...
def test_generation_prepares_only_organizer_checkpoints_before_release(
    ...
    try:
        ...
        assert metadata["current_restatement_reel_ids"] == []
        assert metadata["current_restatement_guard_reel_ids"] == []
        assert metadata["adaptation_fingerprint"] == adaptation_fingerprint
...
def test_recursive_inventory_crosses_multiple_empty_nodes_with_editorial_plans() -> None:
    ...
    try:
        ...
        assert all(
            reel["_selection_self_contained"] is True
            and reel["_selection_is_standalone"] is True
...
def test_current_restatement_inventory_suppression_is_adaptation_scoped(
    monkeypatch,
) -> None:
    conn = _conn()
    now = datetime.now(timezone.utc)
    generation_id = main._create_generation_row(
        ...
        material_id="m1",
        concept_id="c1",
        request_key="current-restatement-source",
        generation_mode="fast",
        retrieval_profile="unified",
    )
    candidate_rows = []
    for index, reel_id in enumerate(("rich", "strict-subset", "application")):
        candidate_rows.append(_insert_generation_reel(
            conn,
            generation_id=generation_id,
            reel_id=reel_id,
            video_id=f"current-restatement-video-{index}",
            created_at=(now + timedelta(seconds=index)).isoformat(),
        ))
    monkeypatch.setattr(
        main,
        ...
        lambda *_args, **kwargs: [
            reel
            for reel in candidate_rows
            if reel["reel_id"] not in set(kwargs.get("exclude_reel_ids") or ())
        ],
    )
    main._persist_generation_lesson_order(
        conn,
    ...
    generation_rows: dict[str, dict] = {}
    generation_ids = main._response_generation_ids(
        conn,
        generation_id,
    ...
    try:
        ...
        assert {
            ...
            for reel in main._generation_job_reels(
                ...
                apply_release_order=False,
                preserve_lesson_order_metadata=True,
                editorial_excluded_reel_ids=same_adaptation_ids,
                editorial_guard_reel_ids=same_adaptation_guards,
            )
        } == {"rich", "application"}
        assert {
            reel["reel_id"]
            for reel in main._generation_job_reels(
                ...
                apply_release_order=False,
                preserve_lesson_order_metadata=True,
                editorial_excluded_reel_ids=changed_adaptation_ids,
                editorial_guard_reel_ids=changed_adaptation_guards,
            )
        } == {"rich", "strict-subset", "application"}
        assert main._current_level_reusable_generation_reel_count(
            conn,
            generation_id=generation_id,
            material_id="m1",
        ...
        candidate_rows[:] = [
            reel for reel in candidate_rows if reel["reel_id"] != "rich"
        ]
        assert {
            reel["reel_id"]
            for reel in main._generation_job_reels(
...
def test_latest_compatible_job_requires_current_adaptation_fingerprint() -> None:
    ...
    params = {
        ...
        "knowledge_level": "beginner",
        "language": "en",
        "exclude_video_ids": ["yt:watched-video"],
        "min_relevance": None,
    ...
    try:
        ...
        canonical_match = main._latest_compatible_generation_job(
            conn,
            material_id="m1",
            learner_id="learner-1",
            concept_id=None,
            content_fingerprint=content_fingerprint,
            request_params={**params, "exclude_video_ids": ["watched-video"]},
        )
        changed_exclusions = main._latest_compatible_generation_job(
            conn,
            material_id="m1",
            learner_id="learner-1",
            concept_id=None,
            ...
            request_params={
                **params,
                "exclude_video_ids": ["watched-video", "newly-watched-video"],
            },
        )
        ...
        assert canonical_match["id"] == prior["id"]
        assert changed is None
        assert changed_exclusions is None
    finally:
        conn.close()
...
def test_generation_job_reels_promote_internal_current_metadata_and_source(
    ...
    try:
        organizer_reels = main._generation_job_reels(
            conn,
            {
        ...
        assert organizer_reels[0]["_selection_self_contained"] is True
        assert organizer_reels[0]["_selection_is_standalone"] is True
        ...
        assert reels[0]["selection_contract_version"] == "quality_silence_v41"
...
def test_generate_submission_persists_adjusted_soft_difficulty_target(
    ...
    try:
        ...
        assert params["request_schema_version"] == "adaptive_clip_concepts_v7"
    finally:
        conn.close()
...
def test_global_provider_retry_keeps_release_lineage_with_fresh_provider_state(
    monkeypatch,
) -> None:
    conn = _conn()
    _patch_request_context(monkeypatch, conn)
    ...
    generated: list[dict] = []
    failed_video_id = "FAILEDVID01"
    try:
        root_response = asyncio.run(main.generate_reels(object(), payload))
        root_job_id = json.loads(root_response.body)["job_id"]
        root_job = generation_jobs.get_job(conn, root_job_id)
        assert root_job
        root_generation_id = main._create_generation_row(
        ...
        seen_reel = _insert_generation_reel(
            conn,
            generation_id=root_generation_id,
            reel_id="global-retry-seen",
            video_id="SEENVIDEO01",
            created_at=now.isoformat(),
        )
        unseen_reel = _insert_generation_reel(
            conn,
            generation_id=root_generation_id,
            reel_id="global-retry-unseen",
            video_id="UNSEENVID01",
            created_at=(now + timedelta(seconds=1)).isoformat(),
        )
        generation_jobs.transition_terminal(
            conn,
            ...
            usage={
                ...
                "consumed_video_ids": ["SEENVIDEO01", "UNSEENVID01"],
            },
        )
        _append_authoritative_release(
            conn,
            job_id=root_job_id,
            reel_ids=[seen_reel["reel_id"], unseen_reel["reel_id"]],
        )
        conn.execute(
            "INSERT INTO learner_reel_progress "
            "(learner_id, reel_id, material_id, max_fraction, scrolled_at, "
        ...
        continuation_response = asyncio.run(main.generate_reels(
            object(),
            ReelsGenerateRequest(
                material_id="m1",
                concept_id="c1",
        ...
        failed_job_id = json.loads(continuation_response.body)["job_id"]
        failed_job = generation_jobs.get_job(conn, failed_job_id)
        assert failed_job
        assert failed_job["source_generation_id"] == root_generation_id
        failed_generation_id = main._create_generation_row(
            conn,
            material_id="m1",
            concept_id="c1",
            request_key=str(failed_job["request_key"]),
            generation_mode="slow",
            retrieval_profile="unified",
            source_generation_id=root_generation_id,
        )
        ...
        generation_jobs.transition_terminal(
            ...
            job_id=failed_job_id,
            status="failed",
            result_generation_id=failed_generation_id,
            error_code="provider_budget_exceeded",
            usage={
                "budget": {
                    "gemini": {
                        "cost_exposure_usd": 2.5,
                        "cost_limit_usd": 2.5,
                    },
                },
                "counters": {
                ...
                "failed_source_attempts": {failed_video_id: 2},
                "provider_calls": [{
                    "provider": "gemini",
                    "operation": "segmentation",
                    "billable_requests": 1,
        ...
        retry_payload = ReelsGenerateRequest(
            ...
            concept_id="c1",
            num_reels=3,
            retry_terminal_job_id=failed_job_id,
        )
        retry_response = asyncio.run(
            main.generate_reels(object(), retry_payload)
        )
        repeated_response = asyncio.run(
            main.generate_reels(object(), retry_payload)
        )
        retry_job_id = json.loads(retry_response.body)["job_id"]
        assert json.loads(repeated_response.body)["job_id"] == retry_job_id
        retry_job = generation_jobs.get_job(conn, retry_job_id)
        assert retry_job
        retry_params = main._job_request_params(retry_job)
        failed_params = main._job_request_params(
            generation_jobs.get_job(conn, failed_job_id) or {}
        )
        assert retry_job["source_generation_id"] == root_generation_id
        assert retry_params["fresh_source_budget"] is True
        assert retry_params["adaptation_fingerprint"] == (
            failed_params["adaptation_fingerprint"]
        )
        assert retry_job["attempt_count"] == 0
        assert json.loads(str(retry_job["usage_json"] or "{}")) == {}
        assert conn.execute(
            "SELECT COUNT(*) FROM reel_generation_jobs "
            "WHERE id NOT IN (?, ?)",
            (root_job_id, failed_job_id),
        ).fetchone()[0] == 1
        assert main._generation_gemini_ledger_exposure(
            conn,
            failed_job_id,
        )["cost_exposure_usd"] > 0
        assert main._generation_gemini_ledger_exposure(
        ...
        def generate_stage(worker_conn, **kwargs) -> None:
            assert kwargs["max_generation_videos"] == (
                main.GENERATION_SOURCE_BUDGETS["slow"]
            )
            assert failed_video_id not in kwargs["exclude_video_ids"]
            fresh_budget = kwargs["generation_context"].usage_payload()["budget"][
                "gemini"
            ]
            assert fresh_budget["cost_exposure_usd"] == 0
            fresh = _insert_generation_reel(
                worker_conn,
                generation_id=str(kwargs["generation_id"]),
                reel_id="global-retry-fresh",
                video_id="FRESHVIDEO1",
                created_at=(now + timedelta(seconds=2)).isoformat(),
            )
            unseen_context = conn.execute(
                "SELECT search_context_json FROM reels WHERE id = ?",
                (unseen_reel["reel_id"],),
            ).fetchone()[0]
            conn.execute(
                ...
                "transcript_snippet, takeaways_json, base_score, generation_id, "
                "search_context_json, created_at) "
                "VALUES ('global-retry-overlap', 'm1', 'c1', 'UNSEENVID01', '', "
                "5, 25, 'Duplicate explanation.', '[]', 1, ?, ?, ?)",
                (
        ...
        def ranked_request_reels(*_args, **kwargs):
            if kwargs["generation_id"] == root_generation_id:
                return [unseen_reel]
            return [unseen_reel, *generated]
        ...
        def order_batch(reels, **kwargs):
            ...
            assert kwargs["required_reel_ids"] == [unseen_reel["reel_id"]]
            assert kwargs["prior_concept_coverage"] == [{
                "concept_id": "c1",
                "concept_family": "",
                "concept_title": "Mitochondria",
            ...
            assert [
                item["release_rank"]
                for item in kwargs["recent_prior_objective_coverage"]
            ] == [0, 1]
            by_id = {reel["reel_id"]: reel for reel in reels}
            ordered_ids = [
                "global-retry-fresh",
                unseen_reel["reel_id"],
            ]
            return mock.Mock(
                reels=[by_id[reel_id] for reel_id in ordered_ids],
        ...
        leased = generation_jobs.lease_job(
            conn,
            job_id=retry_job_id,
            lease_owner="global-lineage-retry-worker",
            now=now + timedelta(seconds=4),
        )
        assert leased and leased["attempt_count"] == 1
        main._run_leased_generation_job(leased, threading.Event())

        completed = generation_jobs.get_job(conn, retry_job_id)
        assert completed and completed["status"] in {"completed", "partial"}
        completed_usage = json.loads(str(completed["usage_json"] or "{}"))
        assert failed_video_id not in completed_usage.get(
            "failed_source_attempts",
            {},
        )
        child_generation_id = str(completed["result_generation_id"])
        child_generation = main._fetch_generation_row(
            conn,
            child_generation_id,
        )
        assert child_generation
        assert child_generation["source_generation_id"] == root_generation_id
        lesson_order = json.loads(str(child_generation["lesson_order_json"]))
        assert lesson_order["adaptation_fingerprint"] == (
            retry_params["adaptation_fingerprint"]
        )
        assert lesson_order["reconciliation_tail_reel_ids"].count(
            unseen_reel["reel_id"]
        ) == 1
        assert "global-retry-overlap" not in lesson_order["ordered_reel_ids"]
        assert main._authoritative_release_reel_ids(
            conn,
            child_generation_id,
        ).count(unseen_reel["reel_id"]) == 1
    finally:
        conn.close()
...
@pytest.mark.parametrize(
    ("adaptation_fingerprint", "current_signals"),
    [
        (
            "after-helpful",
            {"mastered-concept": {"helpful": 1.0, "confusing": 0.0, "adjustment": 0.2}},
        ),
        (
            "after-confusing",
            {"confusing-concept": {"helpful": 0.0, "confusing": 1.0, "adjustment": -0.35}},
        ),
        (
            "after-quiz-correct",
            {"quiz-correct-concept": {"helpful": 0.0, "confusing": 0.0, "adjustment": 0.2}},
        ),
        (
            "after-quiz-wrong",
            {"quiz-wrong-concept": {"helpful": 0.0, "confusing": 0.0, "adjustment": -0.35}},
        ),
    ],
)
def test_feed_full_reservoir_queues_current_adaptation_organizer(
    monkeypatch,
    adaptation_fingerprint: str,
    current_signals: dict[str, dict[str, float]],
) -> None:
    conn = _conn()
    _patch_request_context(monkeypatch, conn)
    completed_at = datetime.now(timezone.utc).isoformat()
    source_generation_id = main._create_generation_row(
        ...
        material_id="m1",
        concept_id=None,
        request_key="before-feedback-request",
        generation_mode="slow",
        retrieval_profile="unified",
    )
    for index in range(main.INITIAL_READY_REEL_TARGET):
        _insert_generation_reel(
            conn,
            generation_id=source_generation_id,
            reel_id=f"before-feedback-reel-{index}",
            video_id=f"before-feedback-video-{index}",
            created_at=completed_at,
        )
    _terminal_job_for_generation(
        conn,
        ...
        request_params={
            "generation_mode": "slow",
            "num_reels": main.INITIAL_READY_REEL_TARGET,
            "exclude_video_ids": [],
            "creative_commons_only": False,
            "min_relevance": None,
            "preferred_video_duration": "any",
    ...
    monkeypatch.setattr(
        main,
        "_learner_adaptation_fingerprint",
        lambda *_args, **_kwargs: adaptation_fingerprint,
    )
    monkeypatch.setattr(
        main,
        "_ranked_request_reels",
    ...
    wake = mock.Mock()
    organizer = mock.Mock(
        side_effect=AssertionError("the HTTP request must not run Gemini")
    )
    monkeypatch.setattr(main, "_wake_generation_worker", wake)
    monkeypatch.setattr(main, "order_lesson_batch", organizer)
    try:
        response = main.feed(
            object(),
            material_id="m1",
            limit=main.INITIAL_READY_REEL_TARGET,
            prefetch=0,
            autofill=True,
            generation_mode="slow",
            exclude_video_ids="watched-source",
        )

        assert [reel["reel_id"] for reel in response["reels"]] == [
            f"before-feedback-reel-{index}"
            for index in range(main.INITIAL_READY_REEL_TARGET)
        ]
        assert response["generation_id"] == source_generation_id
        ...
        assert queued_params["adaptation_fingerprint"] == adaptation_fingerprint
        assert queued_params["exclude_video_ids"] == ["watched-source"]
        assert queued_params["fresh_source_budget"] is False
        organizer.assert_not_called()
        wake.assert_called_once_with()

        organizer_signals: list[dict[str, dict[str, float]]] = []
        source_reels = [
            {"reel_id": f"before-feedback-reel-{index}"}
            for index in range(main.INITIAL_READY_REEL_TARGET)
        ]
        ...
        def order_current(reels, **kwargs):
            organizer_signals.append(kwargs["concept_signals"])
            return mock.Mock(
                reels=list(reels),
                ordered_reel_ids=[reel["reel_id"] for reel in reels],
                assessment_checkpoint_reel_ids=[],
        ...
        acquire = mock.Mock(
            side_effect=AssertionError(
                "a full adaptive reservoir must skip media acquisition"
            )
        )
        monkeypatch.setattr(main.reel_service, "generate_reels", acquire)
        monkeypatch.setattr(main, "order_lesson_batch", order_current)
        monkeypatch.setattr(
            main,
            "_learner_concept_signals",
            lambda *_args, **_kwargs: current_signals,
        )
        monkeypatch.setattr(
            main,
            "_current_level_reusable_generation_reel_count",
            lambda *_args, **_kwargs: main.INITIAL_READY_REEL_TARGET,
        )
        monkeypatch.setattr(
            main,
            "_generation_job_reels",
            lambda *_args, **_kwargs: list(source_reels),
        )
        leased = generation_jobs.lease_job(
            conn,
            job_id=str(queued["id"]),
            lease_owner="after-feedback-worker",
            now=datetime.now(timezone.utc),
        )
        assert leased is not None
        main._run_leased_generation_job(leased, threading.Event())
        ...
        acquire.assert_not_called()
        assert organizer_signals == [current_signals]
        completed = generation_jobs.get_job(conn, str(queued["id"]))
        assert completed is not None
        assert completed["status"] == "completed"
    finally:
        conn.close()
...
def test_cross_request_reuse_requires_current_schema_but_allows_new_adaptation_cursor() -> None:
    conn = _conn()
    params = {
        "generation_mode": "fast",
        "num_reels": 9,
    ...
    try:
        ...
        assert main._verified_cross_request_source_generation(
            ...
            request_params={
                **params,
                "adaptation_fingerprint": "after-feedback",
                "exclude_video_ids": ["watched-video"],
            },
        ) == generation_id
        assert main._verified_cross_request_source_generation(
            conn,
        ...
        stored["exclude_video_ids"] = ["yt:previously-excluded-video"]
        conn.execute(
            "UPDATE reel_generation_jobs SET request_params_json = ? WHERE id = ?",
            (json.dumps(stored), job["id"]),
        )
        assert main._verified_cross_request_source_generation(
            ...
            material_id="m1",
            learner_id="learner-1",
            request_key="cursor-went-backwards",
            concept_id="c1",
            content_fingerprint="same-fingerprint",
            request_params={
                **params,
        ...
        assert main._verified_cross_request_source_generation(
            ...
            material_id="m1",
            learner_id="learner-1",
            request_key="cursor-diverged",
            concept_id="c1",
            content_fingerprint="same-fingerprint",
            request_params={
                **params,
                "adaptation_fingerprint": "after-feedback",
                "exclude_video_ids": ["different-video"],
            },
        ) is None
    finally:
        conn.close()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A watched-source cursor grows monotonically after feedback. The
        # prior verified inventory remains valid evidence for the new editor.
        ("same-fingerprint", {"exclude_video_ids": ["prior-video"]}, True),
