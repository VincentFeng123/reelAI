...
Production requires PostgreSQL plus `SUPADATA_API_KEY` and `GEMINI_API_KEY`. Supadata owns YouTube discovery and timestamped transcript retrieval. Each uncached inspected source gets at most one Gemini 3.1 Pro clip-selection call over the complete timestamped transcript; the YouTube video is not attached to Gemini. The server separately resolves an audio-only stream and measures real quiet intervals for clip boundaries, failing closed when those boundaries cannot be verified. Native YouTube captions may corroborate lexical timing but are not the semantic transcript. Provider responses and model usage are persisted per generation, while successful search evidence is cached for six hours (empty results for 15 minutes) and validated timestamped transcript artifacts for 30 days.
