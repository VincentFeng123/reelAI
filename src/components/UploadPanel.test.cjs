...
test("URL ingestion primes feed snapshots with the current v34 selection contract", () => {
  const filePath = path.join(__dirname, "UploadPanel.tsx");
  const source = fs.readFileSync(filePath, "utf8");
  const sourceFile = ts.createSourceFile(
    filePath,
  ...
  assert.equal(stored["ingest-search:abc"].selectionContractVersion, "quality_silence_v34");
