...
## Corpus Check
- 370 files · ~990,728 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5634 nodes · 14765 edges · 183 communities detected
- Extraction: 67% EXTRACTED · 33% INFERRED · 0% AMBIGUOUS · INFERRED: 4817 edges (avg confidence: 0.5)
...
1. `ReelService` - 187 edges
2. `Unit` - 180 edges
3. `YouTubeService` - 165 edges
4. `Sentence` - 146 edges
5. `Structure` - 101 edges
6. `MediumRegressionTests` - 100 edges
7. `CancellationError` - 99 edges
8. `DatabaseIntegrityError` - 98 edges
9. `ContentMap` - 97 edges
10. `Candidate` - 96 edges
...
- `Snap AI-selected segments to sentence boundaries: start at a sentence start, end` --uses--> `Sentence`  [INFERRED]
  backend/pipeline/refine.py → backend/pipeline/sentences.py
- `BND1: an end sentence is WEAK when it has <3 words or ends on a conjunction/` --uses--> `Sentence`  [INFERRED]
  backend/pipeline/refine.py → backend/pipeline/sentences.py
- `A start sentence is WEAK when it opens the clip mid-thought (leading continuatio` --uses--> `Sentence`  [INFERRED]
  backend/pipeline/refine.py → backend/pipeline/sentences.py
- `BND1 SAFE FALLBACK. Return an end-acceptability predicate: a real terminator is` --uses--> `Sentence`  [INFERRED]
  backend/pipeline/refine.py → backend/pipeline/sentences.py
...
Cohesion: 0.02
Nodes (398): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+390 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (262): AssessmentCancelledError, AssessmentService, _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Validate and persist one private answer-bearing question for a reel., Use a savepoint so multi-row assessment writes cannot be half-persisted., BaseModel, cue_text() (+254 more)
...
Cohesion: 0.03
Nodes (202): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed(), _bare_video_id() (+194 more)
...
Cohesion: 0.02
Nodes (145): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+137 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (35): _difficulty(), ReelService, ClipEngineFeedRefineFeedbackTests, _discover_result(), Tests for feed ranking, feedback score shifts, refinement generation swap, and c, Patch discover + clip at the pipeline aliases; registers cleanup., ranked_feed returns clip-engine reels with a numeric score, descending., helpful feedback raises score; confusing feedback lowers it. (+27 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (55): _build_educational_transcript(), End-to-end search simulation — exercise the full pipeline with mocked YouTube /L, Realistic educational transcript — calculus lecture with 3 topic clusters., SearchSimulationTests, _normalise_cue(), Shared transcript quality validation.  This module is intentionally dependency-f, Check whether a transcript adequately covers a video.      Parameters     ------, Result of :func:`validate_transcript`. (+47 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (139): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+131 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (15): MediumRegressionTests, Bug 1B: final_unique_reels should increment, not stay at max(current, 1)., Bug 1C: _bootstrap_pool_is_weak should not check has_transcript (never set on ca, Bug 1E: Empty search results should only be cached for 15 minutes, not 2 hours., Quality 5A: Known educational channels should get 'known_educational' tier., Niche 4A: Datamuse should be called for opaque topics, with results filtered., Niche 4C: New static topics should be in STATIC_TOPIC_SUBTOPICS., _validated_query_plan() (+7 more)

### Community 10 - "Community 10"
Cohesion: 0.04
Nodes (69): CompletenessContract, ContractElement, Adapter framework core (spec §4, §11).  An *adapter* parameterizes the structure, RoleSpec, _rs(), BaseAdapter, CodingAdapter, Coding / programming / software adapter (Phase 4). (+61 more)

### Community 11 - "Community 11"
Cohesion: 0.04
Nodes (104): FakeAdapter, AnchorAdapter, _built_budget(), _run_repair(), test_closure_budget_clamped_to_ship_cap(), test_closure_budget_smaller_closure_span_wins(), test_repair_expansion_capped_at_ship_cap(), test_repair_expansion_smaller_closure_span_wins() (+96 more)
...
Cohesion: 0.03
Nodes (86): _jsonable(), Run-artifact persistence (W25-G): every assembled run leaves an auditable trail., Best-effort JSON projection: dataclass → asdict, pydantic → model_dump,     dict, Persist one assembled run's plan/arcs/shipped/ledger under     ``<work_dir>/<vid, write_run_artifacts(), best_match(), find_human_clip(), gold_chapters() (+78 more)

### Community 15 - "Community 15"
Cohesion: 0.05
Nodes (85): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _BoundaryPlan, _BoundaryTopic, _call_model() (+77 more)
...
Cohesion: 0.06
Nodes (39): StructuredResult, _assessment(), Strict shipping contract for the guarded Gemini educational selector., _run(), _segs(), _selection_task_tail(), test_application_quality_gates_fail_closed(), test_clip_over_one_eighty_seconds_is_rejected_without_hard_cut() (+31 more)
...
Cohesion: 0.05
Nodes (56): _canonical_feed_url(), canonicalize_youtube_url(), extract_video_id(), normalize_youtube_video_id(), Allowlist channel/playlist paths and discard untrusted query fields., Title/author/duration/thumbnail via yt-dlp (metadata only, no download).     Laz, Return a parsed, credential-free HTTP(S) YouTube URL or ``None``., Resolve a channel/playlist URL to individual YouTube watch URLs (no download). (+48 more)
...
Cohesion: 0.04
Nodes (50): InvalidReferenceError, A supplied material or concept does not exist or the pair is inconsistent., get_ingest_logger(), log_event(), new_trace_id(), Structured logging for the ingestion pipeline.  Every log line emitted from inge, Injects the current trace_id into every log record., Return a logger with the trace_id filter attached.      We install the filter id (+42 more)

### Community 24 - "Community 24"
Cohesion: 0.05
Nodes (59): _better(), _dedupe(), _end_acceptor(), _is_weak_end(), _is_weak_start(), _keep_key(), _last_valid_end(), _merge() (+51 more)
...
Cohesion: 0.1
Nodes (32): _clip(), _private_clip(), _report(), _result(), _segments(), test_all_authoritative_pro_calls_use_selected_baseline(), test_cancelled_worker_never_publishes_late_boundary_or_done_progress(), test_failed_pro_never_exposes_non_green_flash() (+24 more)
...
Cohesion: 0.18
Nodes (26): _complete(), Focused adaptive recall-check service tests (SQLite, no network)., _seed_completed_accuracy(), _seed_reel(), test_answer_reveals_key_then_applies_concept_outcomes_once(), test_attempt_insert_race_reloads_the_winning_choice(), test_backfill_observes_cancellation_before_any_cache_or_question_write(), test_dense_clips_create_and_resume_private_question_session() (+18 more)

### Community 41 - "Community 41"
Cohesion: 0.13
Nodes (15): _atomic_write(), _available_questions(), _check_cancelled(), _completed_reel_target(), _completed_rows(), _ensure_learner_progress(), _information_units(), _latest_cutoff() (+7 more)
...
Cohesion: 0.13
Nodes (13): _conn(), _manual_plan(), test_calculus_basics_timestamped_window_corpus_is_fail_closed(), test_fast_and_slow_plans_use_three_then_six_plus_three_plus_three(), test_final_gate_uses_exact_timestamped_cues_for_native_or_auto_transcripts(), test_missing_required_ai_plan_fields_are_invalid_and_short_cached(), test_model_unavailable_plan_uses_literal_only_and_short_cache(), test_plan_runs_one_structured_expansion_and_caches_by_normalized_literal() (+5 more)
...
Cohesion: 0.31
Nodes (14): _assessment(), _convert(), _plan(), _segs(), test_bad_line_indices_are_rejected_instead_of_clamped(), test_contextual_overlap_below_duplicate_threshold_is_preserved(), test_fine_snap_tightens_boundary_only_after_quote_validation(), test_maps_lines_to_chunk_times_after_required_alignment() (+6 more)
...
Cohesion: 0.23
Nodes (14): _plan(), _run_discover(), test_consensus_expands_one_query_at_a_time(), test_discover_excludes_and_limits(), test_discover_threads_level_to_rank(), test_duplicate_literal_does_not_inflate_consensus(), test_excluded_consensus_does_not_stop_expansion(), test_fast_context_limits_initial_expansion_to_three_queries() (+6 more)
...
Cohesion: 0.27
Nodes (8): _discovery(), _pipeline(), _plan(), test_ingest_topic_counts_topic_rejections_and_persisted_clips(), test_ingest_topic_distinguishes_unavailable_transcript_from_provider_failure(), test_ingest_topic_records_stage_counts_and_propagates_shared_deadline(), _transcript(), _video()
...
Cohesion: 0.27
Nodes (10): assign_speaker(), available(), diarize(), _get_pipeline(), Speaker diarization via pyannote (optional; degrades to single/None speaker).  A, True only if a token is set AND pyannote.audio imports (dep + gated terms are us, Speaker turns for ``audio_path`` (16 kHz mono from download()); [] if unavailabl, The dominant speaker over [start,end] by overlap; nearest-by-midpoint if no over (+2 more)
...
Cohesion: 0.27
Nodes (4): _artifact(), test_transcript_cache_rejects_tombstoned_video(), test_transcript_validation_accepts_auto_mode_artifact(), test_transcript_validation_rejects_nonfinite_and_nonmonotonic_cues()
...
Cohesion: 0.5
Nodes (1): opening_in_context is a required verdict field: a clip that opens mid-thought fa
...
## Knowledge Gaps
- **364 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+359 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 149`** (2 nodes): `ProcessingStepper.tsx`, `ProcessingStepper()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (2 nodes): `ResultsGrid.tsx`, `ResultsGrid()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (2 nodes): `DownloadAll()`, `DownloadAll.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (2 nodes): `VolumetricGlowBackground.tsx`, `VolumetricGlowBackground()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (2 nodes): `useJobStream.ts`, `useJobStream()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 154`** (2 nodes): `test_topics_types.py`, `test_schemas_and_dataclasses_exist()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (2 nodes): `test_provider_cache_db.py`, `test_sqlite_provider_cache_round_trip_and_tombstone_filter()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `ReelService` connect `Community 6` to `Community 1`, `Community 7`, `Community 8`, `Community 9`, `Community 73`, `Community 50`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 137`, `Community 6`, `Community 23`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `Unit` connect `Community 0` to `Community 1`, `Community 4`, `Community 113`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Are the 71 inferred relationships involving `ReelService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ReelService` has 71 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 55 inferred relationships involving `YouTubeService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`YouTubeService` has 55 INFERRED edges - model-reasoned connections that need verification._
- **Are the 140 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 140 INFERRED edges - model-reasoned connections that need verification._