...
## Corpus Check
- 372 files · ~974,578 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5713 nodes · 16976 edges · 184 communities detected
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 6951 edges (avg confidence: 0.5)
...
1. `ReelService` - 209 edges
2. `YouTubeService` - 190 edges
3. `Unit` - 180 edges
4. `Sentence` - 146 edges
5. `CancellationError` - 124 edges
6. `DatabaseIntegrityError` - 123 edges
7. `GenerationContext` - 122 edges
8. `ProviderError` - 118 edges
9. `Structure` - 101 edges
10. `MediumRegressionTests` - 100 edges
...
Cohesion: 0.01
Nodes (499): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+491 more)

### Community 1 - "Community 1"
Cohesion: 0.1
Nodes (256): AssessmentCancelledError, AssessmentService, _BackfillPlan, BaseModel, Bridge: translates clip_engine output into IngestionPipeline._persist_ingest() i, Build IngestSegment from a clip dict; text is in-window transcript or clip title, Token-overlap relevance of a clip against a query.      Returns 1.0 when query i, When query is falsy, return clips unchanged.     Otherwise: annotate each clip w (+248 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (55): Exception, APIStatusError, BadRequestError, chat(), get_client(), parse_groq_duration(), RateBudget, RateLimitError (+47 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (153): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+145 more)
...
Cohesion: 0.02
Nodes (130): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+122 more)

### Community 8 - "Community 8"
Cohesion: 0.03
Nodes (51): _normalise_cue(), Shared transcript quality validation.  This module is intentionally dependency-f, Check whether a transcript adequately covers a video.      Parameters     ------, Result of :func:`validate_transcript`., Convert any supported cue format to ``(start, end, text)``.      Returns ``None`, TranscriptQuality, validate_transcript(), _backoff_delay() (+43 more)
...
Cohesion: 0.04
Nodes (95): BaseAdapter, AnchorAdapter, _built_budget(), _run_repair(), test_closure_budget_clamped_to_ship_cap(), test_closure_budget_smaller_closure_span_wins(), test_repair_expansion_capped_at_ship_cap(), test_repair_expansion_smaller_closure_span_wins() (+87 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (115): _jsonable(), Run-artifact persistence (W25-G): every assembled run leaves an auditable trail., Best-effort JSON projection: dataclass → asdict, pydantic → model_dump,     dict, Persist one assembled run's plan/arcs/shipped/ledger under     ``<work_dir>/<vid, write_run_artifacts(), best_match(), find_human_clip(), gold_chapters() (+107 more)
...
Cohesion: 0.04
Nodes (68): cue_text(), filter_by_query(), pick_best_clip(), relevance_score(), synth_adapter_result(), to_cues(), to_metadata(), to_segment() (+60 more)
...
Cohesion: 0.06
Nodes (55): fetch_chapters(), main(), make_skeleton(), merge_chapters_into_golden(), Dump a golden *skeleton* from the predicted structure, for a human to correct (s, MERGE {"chapters", "chapters_provenance": "creator"} into <golden_dir>/<video_id, Import creator chapters for each id; report + write nothing when a video has non, Creator chapters via the yt_dlp Python API — metadata only     (``extract_info(d (+47 more)
...
Cohesion: 0.1
Nodes (30): _build_cache_key(), _build_cerebras_client(), _build_gemini_module(), _build_groq_client(), _build_ollama_client(), _cerebras_chat(), chat_completion(), _collect_gemini_api_keys() (+22 more)
...
Cohesion: 0.11
Nodes (18): _atomic_write(), _available_questions(), _check_cancelled(), _completed_reel_target(), _completed_rows(), _ensure_learner_progress(), _information_units(), _latest_cutoff() (+10 more)
...
Cohesion: 0.12
Nodes (18): _ai_json(), _conn(), _manual_plan(), test_calculus_basics_timestamped_window_corpus_is_fail_closed(), test_fast_and_slow_plans_use_three_then_six_plus_three_plus_three(), test_final_gate_uses_exact_timestamped_cues_for_native_or_auto_transcripts(), test_intro_to_python_fallback_signature_keeps_the_domain_anchor(), test_long_literal_fallback_preserves_identity_and_bounds_provider_query() (+10 more)
...
Cohesion: 0.17
Nodes (17): _intro_to_python_plan(), _plan(), _run_discover(), test_discover_excludes_and_limits(), test_discover_threads_level_to_rank(), test_excluded_consensus_does_not_stop_expansion(), test_fast_context_limits_initial_expansion_to_three_queries(), test_intro_to_python_searches_literal_before_hd_ai_terms() (+9 more)
...
Cohesion: 0.26
Nodes (9): _discovery(), _pipeline(), _plan(), test_ingest_topic_counts_shared_clip_fetch_timeout_separately(), test_ingest_topic_counts_topic_rejections_and_persisted_clips(), test_ingest_topic_distinguishes_unavailable_transcript_from_provider_failure(), test_ingest_topic_records_stage_counts_and_propagates_shared_deadline(), _transcript() (+1 more)
...
## Knowledge Gaps
- **374 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+369 more)
...
- **Thin community `Community 154`** (2 nodes): `test_topics_types.py`, `test_schemas_and_dataclasses_exist()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (2 nodes): `test_youtube_transport.py`, `test_youtube_impersonation_target_is_supported()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 156`** (2 nodes): `test_provider_cache_db.py`, `test_sqlite_provider_cache_round_trip_and_tombstone_filter()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 157`** (2 nodes): `RootLayout()`, `layout.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 158`** (2 nodes): `Loading()`, `loading.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 159`** (2 nodes): `FullscreenLoadingScreen()`, `FullscreenLoadingScreen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 160`** (2 nodes): `ViewportModalPortal.tsx`, `ViewportModalPortal()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 161`** (2 nodes): `youtubeIframeApi.ts`, `loadYouTubeIframeApi()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 162`** (1 nodes): `next-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 163`** (1 nodes): `tailwind.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 164`** (1 nodes): `postcss.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 165`** (1 nodes): `next.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 166`** (1 nodes): `tailwind.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 169`** (1 nodes): `main.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 170`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 171`** (1 nodes): `SettingsDrawer.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 172`** (1 nodes): `InputCard.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 181`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (1 nodes): `GenerationProgress.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `ReelService` connect `Community 2` to `Community 1`, `Community 7`, `Community 9`, `Community 74`, `Community 51`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 137`, `Community 2`, `Community 21`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `Unit` connect `Community 0` to `Community 1`, `Community 10`, `Community 115`, `Community 28`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Are the 93 inferred relationships involving `ReelService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ReelService` has 93 INFERRED edges - model-reasoned connections that need verification._
- **Are the 80 inferred relationships involving `YouTubeService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`YouTubeService` has 80 INFERRED edges - model-reasoned connections that need verification._
