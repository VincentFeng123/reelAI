...
## Corpus Check
- 407 files · ~1,090,892 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8627 nodes · 34585 edges · 199 communities detected
- Extraction: 46% EXTRACTED · 54% INFERRED · 0% AMBIGUOUS · INFERRED: 18793 edges (avg confidence: 0.5)
...
3. `DatabaseIntegrityError` - 347 edges
...
Cohesion: 0.01
Nodes (637): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+629 more)
...
Cohesion: 0.03
Nodes (395): Each organizer checkpoint is one immediate reel-grounded question., Promote one named level after sustained, broad mastery evidence.          Manual, Resolve reels against the newest learner-owned released lesson plans.          E, Return reached position, checkpoint ID, and organizer cadence control., Consume through the organizer checkpoint, not later session completion., cue_text(), filter_by_query(), pick_best_clip() (+387 more)
...
Cohesion: 0.01
Nodes (303): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+295 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (87): Exception, APIStatusError, BadRequestError, chat(), get_client(), parse_groq_duration(), RateBudget, RateLimitError (+79 more)
...
Cohesion: 0.02
Nodes (188): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+180 more)
...
Cohesion: 0.02
Nodes (193): _acquire_quota_lock(), _attach_reservation_job(), attach_reservation_to_job(), billing_enforcement_enabled(), billing_entitlement_environment(), billing_status(), _day_and_reset(), effective_plan() (+185 more)
...
Cohesion: 0.03
Nodes (49): _age_seconds(), _canonical_json(), MemoryProviderCache, normalize_filters(), normalize_language(), normalize_query(), _parse_time(), Versioned DB caches for Supadata search evidence and timed transcript artifacts. (+41 more)
...
Cohesion: 0.06
Nodes (92): _append_authoritative_release(), _conn(), _insert_generation_reel(), _patch_released_ranked_feed(), _patch_request_context(), _rank_released_feed(), _released_feed_reel(), _released_generation_chain() (+84 more)

### Community 19 - "Community 19"
Cohesion: 0.03
Nodes (46): normalizeSignupEmailForComparison(), onSendVerificationEmail(), onVerifyAccount(), buildCommunitySetInformationParagraphs(), createDraftReelRow(), detectYouTubeDurationWithIframeApi(), draftRowsFromReels(), extractYouTubeVideoId() (+38 more)
...
Cohesion: 0.04
Nodes (25): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+17 more)
...
Cohesion: 0.07
Nodes (33): assessment_checkpoint_reel_ids(), _atomic_write(), _available_questions(), _BackfillPlan, _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows() (+25 more)
...
Cohesion: 0.13
Nodes (32): _acquire_generation_submit_lock(), append_event(), _atomic_write(), build_request_key(), expire_stale_queued_job(), _fail_unclaimable_job(), find_active_job(), get_job() (+24 more)
...
Cohesion: 0.17
Nodes (2): isLikelyIngestUrl(), resolveUnifiedComposerRoute()
...
## Knowledge Gaps
- **442 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+437 more)
...
- **Thin community `Community 172`** (2 nodes): `FullscreenLoadingScreen()`, `FullscreenLoadingScreen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 173`** (2 nodes): `formatAccuracy()`, `RecallCheck.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (2 nodes): `formatResetTime()`, `BillingPlanUsageCard.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `GenerationContext` connect `Community 1` to `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 37`, `Community 8`, `Community 108`, `Community 14`, `Community 16`, `Community 26`, `Community 30`?**
  _High betweenness centrality (0.189) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 1` to `Community 0`, `Community 96`, `Community 2`, `Community 100`, `Community 7`, `Community 9`, `Community 112`, `Community 117`, `Community 87`, `Community 91`, `Community 29`, `Community 95`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 5` to `Community 1`, `Community 2`, `Community 8`, `Community 42`, `Community 21`, `Community 61`?**
...
- **Are the 343 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 343 INFERRED edges - model-reasoned connections that need verification._
