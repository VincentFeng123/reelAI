...
## Corpus Check
- 384 files · ~1,029,576 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6843 nodes · 33634 edges · 262 communities detected
- Extraction: 33% EXTRACTED · 67% INFERRED · 0% AMBIGUOUS · INFERRED: 22578 edges (avg confidence: 0.5)
...
1. `GenerationContext` - 554 edges
2. `CancellationError` - 517 edges
3. `DatabaseIntegrityError` - 502 edges
4. `ProviderError` - 498 edges
5. `RateLimitedError` - 461 edges
6. `ReelService` - 370 edges
7. `IngestTranscriptCue` - 315 edges
8. `EmbeddingService` - 309 edges
9. `IngestError` - 283 edges
10. `UnsupportedSourceError` - 277 edges
...
Cohesion: 0.01
Nodes (701): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+693 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (500): Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Use a savepoint so multi-row assessment writes cannot be half-persisted., Use a savepoint so multi-row assessment writes cannot be half-persisted. (+492 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (149): _difficulty(), _get_punct_pipeline(), _has_selection_contract(), _importance_ranker_enabled(), _is_short_leaf_topic(), QueryCandidate, ReelService, _selection_metadata() (+141 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (236): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+228 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (218): ABC, _atomic_write(), _available_questions(), _BackfillPlan, _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows() (+210 more)

### Community 5 - "Community 5"
Cohesion: 0.4
Nodes (247): AssessmentCancelledError, AssessmentService, DownloadError, EngineError, yt-dlp or ffmpeg failed to retrieve / decode the source media., The source has no usable timestamped transcript., Transcript was usable but no valid clip window could be selected., Base error for the clip engine. (+239 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (215): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed(), _bare_video_id() (+207 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (166): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+158 more)
...
Cohesion: 0.03
Nodes (127): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _BoundaryPlan, _BoundaryRepairCandidate (+119 more)
...
Cohesion: 0.05
Nodes (97): FakeAdapter, _cand(), _clean_verdict(), _coverage_units(), _linked(), _mock_judge(), _sev_settings(), _sev_specs() (+89 more)

### Community 13 - "Community 13"
Cohesion: 0.04
Nodes (49): StructuredResult, _assessment(), Strict shipping contract for the guarded Gemini educational selector., _run(), _segs(), _selection_task_tail(), test_clip_over_one_eighty_seconds_is_preserved_without_hard_cut(), test_complete_clips_of_any_duration_survive() (+41 more)
...
Cohesion: 0.13
Nodes (35): _conn(), _insert_generation_reel(), _patch_request_context(), _set_reel_boundary_state(), _terminal_job_for_generation(), test_authoritative_job_inventory_drops_candidates_absent_from_final_rank(), test_completed_job_status_and_replay_drop_currently_invalid_candidate(), test_cross_level_reservoir_rejects_unverified_or_implicit_surface_rows() (+27 more)
...
Cohesion: 0.14
Nodes (27): _assessment(), _convert(), _plan(), _segs(), test_bad_line_indices_are_rejected_instead_of_clamped(), test_boundary_repair_without_explicit_clip_limit_does_not_compare_none(), test_chemistry_fast_path_closes_two_cues_of_missing_setup(), test_clip_limit_uses_relevance_then_stable_chronology() (+19 more)

### Community 30 - "Community 30"
Cohesion: 0.16
Nodes (28): _discovery(), _pipeline(), _plan(), _quality_clip(), test_acoustic_failure_is_stored_but_never_emitted(), test_acoustic_failures_fail_closed_without_pro_repair(), test_candidate_plan_prioritizes_level_eligible_then_difficulty(), test_filter_rejects_non_standalone_dependent_instead_of_repairing_it() (+20 more)
...
Cohesion: 0.27
Nodes (9): _basic_stem_variants(), chunk_transcript(), lexical_overlap_score(), merge_adjacent(), normalize_terms(), Generate stem variants for better lexical recall without a heavy stemmer., select_segments(), _normalize_rows() (+1 more)
...
Cohesion: 0.36
Nodes (8): _proposal(), test_carolingian_visual_dependent_span_is_rejected(), test_context_expands_beyond_eight_cues_and_thirty_seconds(), test_duration_settings_do_not_change_a_complete_clip(), test_genetic_drift_callback_end_extends_through_its_explanation(), test_only_topic_relevance_is_a_numeric_hard_gate(), test_rephrased_facet_is_deduped_but_distinct_facet_survives(), test_short_topic_sentence_with_anaphoric_explanation_remains_a_valid_start()
...
Cohesion: 0.36
Nodes (7): _is_framing_or_question(), is_onset(), opens_mid_thought(), Discourse-onset detection (text-only, genre-independent).  Decides whether a sen, A leading marker is fine when the sentence stands on its own as new framing/a qu, True when this sentence, as a clip's first line, drops the viewer mid-thought., _words()
...
## Knowledge Gaps
- **554 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+549 more)
...
- **Thin community `Community 188`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (1 nodes): `Production regression: ``advanced ATP`` retrieved SAP S/4HANA for a         cell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (1 nodes): `Text/file materials may have no subject tag. Their sibling concepts         stil`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (1 nodes): `Old completed inventories are replayed without regeneration, so the         serv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `Lowercased/stripped level name; absent -> 'beginner'; unknown -> ValueError.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `The difficulty the feed should aim at RIGHT NOW for this material.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `Accept only rows with explicit strict energy-threshold evidence.      Measured e`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `Gemini 3.5 single-pass prompt: policy/examples, context, task last.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `Pre-router production prompt used by ``production_pro_v0``.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Return ``(boundary_time, score)`` for a high-confidence word alignment.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Move outward to the nearest qualifying caption-gap midpoint.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `Validate assessment evidence only against the real clip transcript.      ``evide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `Compatibility helper used by focused conversion tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `Run one immutable benchmark profile without production routing.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `Return guarded educational clips while preserving the existing public tuple.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `Ensure min <= target <= max. Swap min/max if inverted and clamp the target     i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `Reject the entire artifact on malformed, non-finite, or non-monotonic cues.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `Deterministic cache used by focused provider tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `Portable DB repository; coordinated migrations create the dedicated tables.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `The intentionally simple merge used by the practice retrieval path.      A video`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `Fetch timed cues and interpolate the word timeline used by practice.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `Caller passes platforms=["yt","ig","tt"]; result must have platforms==["yt"]`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `resolve_feed_urls extracts watch URLs from yt_dlp entries.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `resolve_feed_urls truncates to max_items.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `resolve_feed_urls swallows exceptions and returns [].`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `Gemini 3.5 single-pass prompt: policy/examples, context, task last.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Pre-router production prompt used by ``production_pro_v0``.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `Return ``(boundary_time, score)`` for a high-confidence word alignment.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Move outward to the nearest qualifying caption-gap midpoint.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Validate assessment evidence only against the real clip transcript.      ``evide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `Compatibility helper used by focused conversion tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `Run one immutable benchmark profile without production routing.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (1 nodes): `Return guarded educational clips while preserving the existing public tuple.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Add knowledge-level columns to pre-existing DBs (sqlite lacks     ADD COLUMN IF`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (1 nodes): `Add one-pass learning-content columns to pre-existing reel tables.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (1 nodes): `Idempotently upgrade legacy SQLite tables for durable generation work.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `Idempotently upgrade legacy PostgreSQL tables for durable generation work.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 249`** (1 nodes): `Connect to Postgres, retrying transient startup errors.      Returns an open psy`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `Return a reusable PostgreSQL connection, or create a fresh one.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `Return a connection to the pool (or close it if the pool is full).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): `Plain INSERT — raises DatabaseIntegrityError on unique constraint violation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 253`** (1 nodes): `Execute a non-SELECT statement (INSERT/UPDATE/DELETE) and return the number of a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 255`** (1 nodes): `resolve_feed_urls extracts watch URLs from yt_dlp entries.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 256`** (1 nodes): `resolve_feed_urls truncates to max_items.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `resolve_feed_urls swallows exceptions and returns [].`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 258`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 259`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 260`** (1 nodes): `Compatibility alias for the old keyless expansion entry point.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `Fetch timestamped cues without downloading media or inventing word times.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ReelService` connect `Community 2` to `Community 1`, `Community 5`, `Community 74`, `Community 50`, `Community 51`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 0` to `Community 1`, `Community 7`, `Community 8`, `Community 105`, `Community 10`, `Community 76`, `Community 111`, `Community 79`, `Community 17`, `Community 82`, `Community 83`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 0`, `Community 2`, `Community 5`, `Community 141`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Are the 536 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 536 INFERRED edges - model-reasoned connections that need verification._
- **Are the 514 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 514 INFERRED edges - model-reasoned connections that need verification._
- **Are the 498 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 498 INFERRED edges - model-reasoned connections that need verification._
- **Are the 484 inferred relationships involving `ProviderError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ProviderError` has 484 INFERRED edges - model-reasoned connections that need verification._