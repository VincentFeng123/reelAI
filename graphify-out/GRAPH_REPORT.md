...
## Corpus Check
- 383 files · ~1,024,803 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6507 nodes · 25272 edges · 242 communities detected
- Extraction: 43% EXTRACTED · 57% INFERRED · 0% AMBIGUOUS · INFERRED: 14314 edges (avg confidence: 0.5)
...
1. `GenerationContext` - 335 edges
2. `CancellationError` - 314 edges
3. `ReelService` - 301 edges
4. `DatabaseIntegrityError` - 299 edges
5. `ProviderError` - 295 edges
6. `RateLimitedError` - 258 edges
7. `YouTubeService` - 244 edges
8. `Sentence` - 195 edges
9. `EmbeddingService` - 187 edges
10. `Unit` - 180 edges
...
Cohesion: 0.01
Nodes (598): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+590 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (457): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Use a savepoint so multi-row assessment writes cannot be half-persisted. (+449 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (146): effective_level_target(), normalize_knowledge_level(), Knowledge-level semantics: level names, difficulty-scale mapping, and the effect, Lowercased/stripped level name; absent -> 'beginner'; unknown -> ValueError., The difficulty the feed should aim at RIGHT NOW for this material., _difficulty(), _get_punct_pipeline(), _has_selection_contract() (+138 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (212): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed(), _bare_video_id() (+204 more)

### Community 4 - "Community 4"
Cohesion: 0.37
Nodes (211): AssessmentCancelledError, AssessmentService, BaseModel, EmbeddingService, DownloadError, EngineError, IngestError, Base class for all ingestion failures. (+203 more)
...
Cohesion: 0.02
Nodes (147): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+139 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (126): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+118 more)
...
Cohesion: 0.02
Nodes (121): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+113 more)

### Community 10 - "Community 10"
Cohesion: 0.03
Nodes (94): CompletenessContract, ContractElement, Adapter framework core (spec §4, §11).  An *adapter* parameterizes the structure, RoleSpec, _rs(), BaseAdapter, CodingAdapter, Coding / programming / software adapter (Phase 4). (+86 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (133): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _BoundaryPlan, _BoundaryRepairCandidate (+125 more)
...
Cohesion: 0.04
Nodes (60): _estimate_tokens(), _groq_json_schema(), _json_instruction(), llm_json(), llm_json_result(), One provider-abstracted structured-JSON LLM call.  Every structure-first pass (c, A strict 'return only JSON of this shape' instruction for the Groq json_object p, Best-effort OpenAI-compatible json_schema wrapper around a Pydantic model. (+52 more)
...
Cohesion: 0.05
Nodes (83): _apply_verdict(), _cut_segment(), EdgeVerdict, _probe_clip(), VID2 — edge probe (Tier 1 video judge): advisory audio-boundary check on shipped, Advisory edge probe over the shipped survivors — mutates each spec's warnings in, Cut a small [start_s, start_s+dur_s] mp4 segment via ffmpeg (cut.build_cmd) into, Cut the head + tail, send ONE inline video judge call, return the parsed EdgeVer (+75 more)
...
Cohesion: 0.08
Nodes (41): _clip(), _private_clip(), _report(), _result(), _segments(), test_all_authoritative_pro_calls_use_selected_baseline(), test_boundary_profile_repairs_bad_model_quote_from_cited_cue(), test_cancelled_worker_never_publishes_late_boundary_or_done_progress() (+33 more)
...
Cohesion: 0.15
Nodes (26): _assessment(), _convert(), _plan(), _segs(), test_bad_line_indices_are_rejected_instead_of_clamped(), test_chemistry_fast_path_closes_two_cues_of_missing_setup(), test_clip_limit_uses_importance_before_chronology(), test_contextual_overlap_below_duplicate_threshold_is_preserved() (+18 more)
...
Cohesion: 0.17
Nodes (26): _discovery(), _pipeline(), _plan(), _quality_clip(), test_acoustic_failure_is_stored_but_never_emitted(), test_acoustic_failures_fail_closed_without_pro_repair(), test_each_quality_axis_has_an_independent_hard_point_seven_five_gate(), test_filter_rejects_non_standalone_dependent_instead_of_repairing_it() (+18 more)

### Community 33 - "Community 33"
Cohesion: 0.19
Nodes (31): _count_judge(), _flagged_at_repair_then(), _mk_candidate(), _one_unit_structure(), _prereq_judge(), _prereq_settings(), _prereq_structure(), _same_model() (+23 more)

### Community 34 - "Community 34"
Cohesion: 0.16
Nodes (27): _conn(), _insert_generation_reel(), _patch_request_context(), _set_reel_boundary_state(), _terminal_job_for_generation(), test_authoritative_job_inventory_drops_candidates_absent_from_final_rank(), test_completed_job_status_and_replay_drop_currently_invalid_candidate(), test_cross_level_reservoir_rejects_unverified_or_implicit_surface_rows() (+19 more)
...
Cohesion: 0.42
Nodes (9): _clip(), _key(), test_segment_cache_accepts_complete_clips_longer_than_180_seconds(), test_segment_cache_key_tracks_transcript_topic_and_policy(), test_segment_cache_revalidates_public_clip_contract(), test_segment_cache_round_trips_empty_qualifying_result(), test_segment_cache_sqlite_round_trip_expiry_and_tombstone(), test_segment_cache_treats_requested_max_as_preference() (+1 more)
...
Cohesion: 0.36
Nodes (8): _proposal(), test_carolingian_visual_dependent_span_is_rejected(), test_context_expands_beyond_eight_cues_and_thirty_seconds(), test_duration_settings_do_not_change_a_complete_clip(), test_each_quality_floor_is_independently_hard(), test_genetic_drift_callback_end_extends_through_its_explanation(), test_rephrased_facet_is_deduped_but_distinct_facet_survives(), test_short_topic_sentence_with_anaphoric_explanation_remains_a_valid_start()
...
## Knowledge Gaps
- **473 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+468 more)
...
- **Thin community `Community 188`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (1 nodes): `Gemini 3.5 single-pass prompt: policy/examples, context, task last.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `Pre-router production prompt used by ``production_pro_v0``.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (1 nodes): `Return ``(boundary_time, score)`` for a high-confidence word alignment.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (1 nodes): `Move outward to the nearest qualifying caption-gap midpoint.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (1 nodes): `Validate assessment evidence only against the real clip transcript.      ``evide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (1 nodes): `Compatibility helper used by focused conversion tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (1 nodes): `Run one immutable benchmark profile without production routing.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `Return guarded educational clips while preserving the existing public tuple.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `Ensure min <= target <= max. Swap min/max if inverted and clamp the target     i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Reject the entire artifact on malformed, non-finite, or non-monotonic cues.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Deterministic cache used by focused provider tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Portable DB repository; coordinated migrations create the dedicated tables.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `The intentionally simple merge used by the practice retrieval path.      A video`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `Fetch timed cues and interpolate the word timeline used by practice.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Caller passes platforms=["yt","ig","tt"]; result must have platforms==["yt"]`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `resolve_feed_urls extracts watch URLs from yt_dlp entries.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `resolve_feed_urls truncates to max_items.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `resolve_feed_urls swallows exceptions and returns [].`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `Gemini 3.5 single-pass prompt: policy/examples, context, task last.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `Pre-router production prompt used by ``production_pro_v0``.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `Return ``(boundary_time, score)`` for a high-confidence word alignment.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `Move outward to the nearest qualifying caption-gap midpoint.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Validate assessment evidence only against the real clip transcript.      ``evide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `Compatibility helper used by focused conversion tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `Run one immutable benchmark profile without production routing.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `Return guarded educational clips while preserving the existing public tuple.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `Add knowledge-level columns to pre-existing DBs (sqlite lacks     ADD COLUMN IF`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `Add one-pass learning-content columns to pre-existing reel tables.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `Idempotently upgrade legacy SQLite tables for durable generation work.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `Idempotently upgrade legacy PostgreSQL tables for durable generation work.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 237`** (1 nodes): `resolve_feed_urls swallows exceptions and returns [].`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Compatibility alias for the old keyless expansion entry point.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Fetch timestamped cues without downloading media or inventing word times.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ReelService` connect `Community 2` to `Community 1`, `Community 4`, `Community 6`, `Community 78`, `Community 15`, `Community 53`, `Community 59`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 0` to `Community 1`, `Community 5`, `Community 6`, `Community 105`, `Community 11`, `Community 111`, `Community 80`, `Community 82`, `Community 19`, `Community 84`, `Community 85`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 2`, `Community 4`, `Community 141`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Are the 317 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 317 INFERRED edges - model-reasoned connections that need verification._
- **Are the 311 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 311 INFERRED edges - model-reasoned connections that need verification._
- **Are the 184 inferred relationships involving `ReelService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ReelService` has 184 INFERRED edges - model-reasoned connections that need verification._
- **Are the 295 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 295 INFERRED edges - model-reasoned connections that need verification._