...
## Corpus Check
- 407 files · ~1,090,292 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8614 nodes · 34566 edges · 197 communities detected
- Extraction: 46% EXTRACTED · 54% INFERRED · 0% AMBIGUOUS · INFERRED: 18783 edges (avg confidence: 0.5)
...
1. `GenerationContext` - 759 edges
2. `Sentence` - 532 edges
3. `DatabaseIntegrityError` - 337 edges
4. `CancellationError` - 334 edges
5. `ProviderError` - 308 edges
6. `ReelService` - 294 edges
7. `RateLimitedError` - 271 edges
8. `EmbeddingService` - 243 edges
9. `IngestError` - 223 edges
10. `UnsupportedSourceError` - 217 edges
...
Cohesion: 0.01
Nodes (736): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+728 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (653): Enum, _acquire_selector_slot(), _apply_enrichment(), _Assessment, _AssessmentDraft, _AtomicDeclarative, _audit_pro_boundaries(), _authoritative_pro() (+645 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (435): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Prepare one private recall question for every reel in a released batch., Each organizer checkpoint is one immediate reel-grounded question., Promote one named level after sustained, broad mastery evidence.          Manual, Build an immediate transcript-only recall question without provider I/O., Validate and persist one private answer-bearing question for a reel., Resolve reels against the newest learner-owned released lesson plans.          E (+427 more)
...
Cohesion: 0.02
Nodes (249): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _apply_generation_lesson_order(), _authoritative_release_reel_ids() (+241 more)
...
Cohesion: 0.38
Nodes (207): AssessmentCancelledError, AssessmentService, DailySearchLimitReached, BillingAccountNotFoundError, BillingConfigurationError, BillingVerificationError, DuplicateSubscriptionError, EngineError (+199 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (172): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+164 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (185): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+177 more)
...
Cohesion: 0.02
Nodes (147): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+139 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (56): _build_educational_transcript(), End-to-end search simulation — exercise the full pipeline with mocked YouTube /L, Realistic educational transcript — calculus lecture with 3 topic clusters., SearchSimulationTests, _normalise_cue(), Shared transcript quality validation.  This module is intentionally dependency-f, Check whether a transcript adequately covers a video.      Parameters     ------, Result of :func:`validate_transcript`. (+48 more)
...
Cohesion: 0.03
Nodes (135): _cancel_remote_stripe_subscription(), cancel_stripe_for_account(), construct_stripe_event(), create_stripe_checkout(), create_stripe_portal(), _epoch_seconds_iso(), _id(), lock_billing_account() (+127 more)

### Community 15 - "Community 15"
Cohesion: 0.02
Nodes (43): _age_seconds(), _canonical_json(), MemoryProviderCache, normalize_filters(), normalize_language(), normalize_query(), _parse_time(), Versioned DB caches for Supadata search evidence and timed transcript artifacts. (+35 more)
...
Cohesion: 0.05
Nodes (119): answerAssessmentQuestion(), ApiError, apiUrl(), askStudyChat(), buildApiError(), buildGenerateReelsRequestBody(), cancelGenerationJob(), canonicalRequestIdentity() (+111 more)

### Community 19 - "Community 19"
Cohesion: 0.02
Nodes (48): normalizeSignupEmailForComparison(), onSendVerificationEmail(), onVerifyAccount(), buildCommunitySetInformationParagraphs(), createDraftReelRow(), detectYouTubeDurationWithIframeApi(), draftRowsFromReels(), extractYouTubeVideoId() (+40 more)

### Community 20 - "Community 20"
Cohesion: 0.03
Nodes (95): crop_clip(), crop_clip_local(), crop_highlights(), crop_highlights_local(), _cut_subclip(), _ratio(), Local clipping: ffmpeg subclip + OpenCV face-aware vertical crop.  Two stages pe, Submit one autocrop job and return the URL of the rendered short. (+87 more)

### Community 21 - "Community 21"
Cohesion: 0.06
Nodes (91): _append_authoritative_release(), _conn(), _insert_generation_reel(), _patch_released_ranked_feed(), _patch_request_context(), _rank_released_feed(), _released_feed_reel(), _released_generation_chain() (+83 more)
...
Cohesion: 0.05
Nodes (67): _absolute_words(), align_groq_edge_anchor(), _cancelled(), _create_client(), _field(), _finite_number(), _language(), Bounded audio-only Groq timing for one already-selected source window.  The adap (+59 more)
...
Cohesion: 0.1
Nodes (25): assessment_checkpoint_reel_ids(), _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows(), _ensure_learner_progress() (+17 more)
...
Cohesion: 0.13
Nodes (33): _acquire_quota_lock(), _attach_reservation_job(), attach_reservation_to_job(), billing_enforcement_enabled(), billing_entitlement_environment(), billing_status(), _day_and_reset(), effective_plan() (+25 more)
...
Cohesion: 0.14
Nodes (30): _acquire_generation_submit_lock(), append_event(), _atomic_write(), build_request_key(), expire_stale_queued_job(), _fail_unclaimable_job(), find_active_job(), get_job() (+22 more)
...
Cohesion: 0.27
Nodes (21): _cue(), _plan(), _report(), test_binary_comparison_keeps_only_the_explicit_requested_relation(), test_conditional_complement_is_kept_despite_caption_punctuation(), test_context_expansion_trims_to_latest_topic_anchored_sentence(), test_embedded_subject_split_across_cues_keeps_its_predicate_and_condition(), test_embedded_subject_split_expands_through_its_predicate() (+13 more)
...
## Knowledge Gaps
- **433 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+428 more)
...
- **Thin community `Community 170`** (2 nodes): `FullscreenLoadingScreen()`, `FullscreenLoadingScreen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 171`** (2 nodes): `formatAccuracy()`, `RecallCheck.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 172`** (2 nodes): `formatResetTime()`, `BillingPlanUsageCard.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `GenerationContext` connect `Community 1` to `Community 2`, `Community 34`, `Community 3`, `Community 5`, `Community 6`, `Community 8`, `Community 107`, `Community 15`, `Community 20`, `Community 27`, `Community 30`?**
  _High betweenness centrality (0.189) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 1` to `Community 0`, `Community 98`, `Community 2`, `Community 7`, `Community 9`, `Community 111`, `Community 115`, `Community 87`, `Community 91`, `Community 29`, `Community 94`, `Community 95`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 5` to `Community 1`, `Community 2`, `Community 6`, `Community 40`, `Community 11`, `Community 23`, `Community 61`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Are the 740 inferred relationships involving `GenerationContext` (e.g. with `_StrictModel` and `_UncertaintyReason`) actually correct?**
  _`GenerationContext` has 740 INFERRED edges - model-reasoned connections that need verification._
- **Are the 526 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 526 INFERRED edges - model-reasoned connections that need verification._
- **Are the 333 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 333 INFERRED edges - model-reasoned connections that need verification._
- **Are the 331 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 331 INFERRED edges - model-reasoned connections that need verification._