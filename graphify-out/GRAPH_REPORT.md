...
## Corpus Check
- 388 files · ~1,072,140 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6707 nodes · 18281 edges · 203 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 6375 edges (avg confidence: 0.5)
...
3. `Unit` - 180 edges
4. `GenerationContext` - 165 edges
5. `CancellationError` - 145 edges
6. `DatabaseIntegrityError` - 136 edges
7. `ProviderError` - 133 edges
8. `YouTubeService` - 132 edges
9. `MediumRegressionTests` - 104 edges
10. `RateLimitedError` - 103 edges
...
Cohesion: 0.01
Nodes (509): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+501 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (345): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Use a savepoint so multi-row assessment writes cannot be half-persisted., cue_text() (+337 more)
...
Cohesion: 0.02
Nodes (49): BaseSettings, get_settings(), Settings, _difficulty(), _get_punct_pipeline(), _has_selection_contract(), _importance_ranker_enabled(), _is_short_leaf_topic() (+41 more)
...
Cohesion: 0.14
Nodes (141): AssessmentCancelledError, AssessmentService, _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows() (+133 more)
...
Cohesion: 0.03
Nodes (160): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _BoundaryPlan, _BoundaryRepairCandidate (+152 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (88): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+80 more)
...
Cohesion: 0.05
Nodes (94): _call_telemetry(), _cancel_requested(), _create_client(), _default_gemini3_thinking_level(), _error_code(), _exception_headers(), _field(), _finish_reason() (+86 more)
...
Cohesion: 0.08
Nodes (53): _discovery(), _one_cue_selector_result(), _pipeline(), _pipeline_with_semantic(), _plan(), _quality_clip(), test_acoustic_boundary_plan_fails_closed_for_missing_cue_ids(), test_acoustic_result_crossing_prior_unselected_speech_falls_back_to_transcript() (+45 more)
...
## Knowledge Gaps
- **394 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+389 more)
...
- **Why does `ReelService` connect `Community 3` to `Community 1`, `Community 6`, `Community 7`, `Community 10`, `Community 46`, `Community 88`, `Community 62`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 0` to `Community 1`, `Community 98`, `Community 99`, `Community 4`, `Community 5`, `Community 8`, `Community 115`, `Community 24`, `Community 91`, `Community 120`, `Community 94`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 66`, `Community 3`, `Community 6`, `Community 10`, `Community 154`?**
...
- **Are the 147 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 147 INFERRED edges - model-reasoned connections that need verification._