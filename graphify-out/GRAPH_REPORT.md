...
## Corpus Check
- 392 files · ~1,170,882 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 11517 nodes · 55965 edges · 345 communities detected
- Extraction: 24% EXTRACTED · 76% INFERRED · 0% AMBIGUOUS · INFERRED: 42345 edges (avg confidence: 0.5)
...
1. `Sentence` - 3222 edges
2. `GenerationContext` - 1188 edges
3. `CancellationError` - 1149 edges
4. `DatabaseIntegrityError` - 1130 edges
5. `ProviderError` - 1127 edges
6. `RateLimitedError` - 1087 edges
7. `IngestTranscriptCue` - 859 edges
8. `EmbeddingService` - 617 edges
9. `IngestError` - 597 edges
10. `UnsupportedSourceError` - 591 edges
...
- `Sentence` --uses--> `Recognize a complete question introduced by a prepositional frame.`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/gemini_segment.py
- `Pre-router production prompt used by ``production_pro_v0``.` --uses--> `Sentence`  [INFERRED]
  backend/pipeline/gemini_segment.py → backend/pipeline/sentences.py
- `Sentence` --uses--> `Distinguish a backward reference from a locally restated example setup.`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/gemini_segment.py
...
Cohesion: 0.09
Nodes (402): Bridge: translates clip_engine output into IngestionPipeline._persist_ingest() i, Construct the canonical YouTube source reference used by persistence., Build IngestSegment from a clip dict; text is in-window transcript or clip title, Token-overlap relevance of a clip against a query.      Returns 1.0 when query i, When query is falsy, return clips unchanged.     Otherwise: annotate each clip w, Pick the single best clip for a single-clip endpoint: prefer clips whose     dur, Convert transcript segments to IngestTranscriptCues, skipping blank text., Return joined text of segments whose window strictly overlaps (t0, t1).      A s (+394 more)

### Community 4 - "Community 4"
Cohesion: 0.33
Nodes (317): AssessmentCancelledError, AssessmentService, EngineError, Base error for the clip engine., GenerationQueueFullError, JobLeaseLostError, Generate a verification code, store its hash, and return the plaintext code., Generate a verification code, store its hash, and return the plaintext code. (+309 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (299): Enum, _acquire_selector_slot(), _apply_enrichment(), _AtomicDeclarative, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _boundary_selector_content() (+291 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (232): ABC, Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed() (+224 more)
...
Cohesion: 0.01
Nodes (130): cue_text(), filter_by_query(), pick_best_clip(), relevance_score(), synth_adapter_result(), to_cues(), to_metadata(), to_segment() (+122 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (167): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+159 more)
...
Cohesion: 0.01
Nodes (201): ProviderBudgetExceededError, _AssessmentDraft, _BoundaryPlan, _BoundaryRepairCandidate, _BoundaryRepairItem, _BoundaryRepairPlan, _BoundaryTopic, _CardEnrichmentItem (+193 more)
...
Cohesion: 0.02
Nodes (132): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+124 more)

### Community 17 - "Community 17"
Cohesion: 0.02
Nodes (112): _compact_plan(), _intent_plan(), _proposal(), test_articleless_look_at_visual_noun_remains_visual_dependent(), test_assignment_leadin_is_retained_when_its_cue_contains_grounded_teaching(), test_back_to_navigation_does_not_delete_worked_example_setup(), test_bare_look_at_this_remains_visual_dependent(), test_boundary_quote_reanchoring_never_discards_substantive_context() (+104 more)

### Community 18 - "Community 18"
Cohesion: 0.03
Nodes (120): _call_telemetry(), _cancel_requested(), count_request_tokens(), _create_client(), _default_gemini3_thinking_level(), _error_code(), _exception_headers(), _field() (+112 more)
...
## Knowledge Gaps
- **3413 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+3408 more)
...
- **Thin community `Community 195`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `GenerationProgress.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `Extract an HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 201`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `A Gemini FileData Part for one canonical public YouTube video URL.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 210`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 213`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `Provider metadata for one logical Gemini structured-content call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `A typed Gemini failure carrying telemetry collected before failure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 228`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 237`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Exact cue 0-30 slice from the 129-cue production Supadata artifact.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 292`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 294`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 295`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 300`** (1 nodes): `Align a uniquely quoted partial edge to explicit lexical onsets.      ``start```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 315`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 316`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 317`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 318`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 325`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 326`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 327`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 328`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 329`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 330`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 331`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 332`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 333`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 334`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 335`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 336`** (1 nodes): `Production regression: ``advanced ATP`` retrieved SAP S/4HANA for a         cell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 337`** (1 nodes): `Text/file materials may have no subject tag. Their sibling concepts         stil`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 338`** (1 nodes): `Old completed inventories are replayed without regeneration, so the         serv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 339`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 340`** (1 nodes): `Return the first letter/digit index, ignoring opening quotes/brackets.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 341`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 342`** (1 nodes): `Reject only question references without context earlier in the same cue.      Ca`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 343`** (1 nodes): `Find opening references without a lexical antecedent in accumulated context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 344`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Sentence` connect `Community 0` to `Community 96`, `Community 1`, `Community 2`, `Community 97`, `Community 5`, `Community 101`, `Community 9`, `Community 10`, `Community 12`, `Community 114`, `Community 115`, `Community 118`, `Community 30`, `Community 89`, `Community 93`, `Community 62`?**
...
- **Why does `ProviderBudgetExceededError` connect `Community 12` to `Community 2`, `Community 4`, `Community 5`, `Community 8`, `Community 18`, `Community 62`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Are the 3216 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 3216 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1169 inferred relationships involving `GenerationContext` (e.g. with `_HTTPStatusError` and `_ModelSequence`) actually correct?**
  _`GenerationContext` has 1169 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1146 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 1146 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1126 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 1126 INFERRED edges - model-reasoned connections that need verification._