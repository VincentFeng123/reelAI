...
2. `YouTubeService` - 465 edges
3. `DatabaseIntegrityError` - 437 edges
4. `EmbeddingService` - 404 edges
5. `IngestTranscriptCue` - 390 edges
6. `RateLimitedError` - 311 edges
...
Cohesion: 0.05
Nodes (44): _Assessment, _clean_list(), _clean_text(), _contains_tokens(), _cue_batches(), _cue_id(), _estimate_tokens(), _grounded_takeaways() (+36 more)
...
- **Are the 365 inferred relationships involving `ReelService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ReelService` has 365 INFERRED edges - model-reasoned connections that need verification._
- **Are the 355 inferred relationships involving `YouTubeService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`YouTubeService` has 355 INFERRED edges - model-reasoned connections that need verification._
- **Are the 433 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 433 INFERRED edges - model-reasoned connections that need verification._
- **Are the 395 inferred relationships involving `EmbeddingService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`EmbeddingService` has 395 INFERRED edges - model-reasoned connections that need verification._