...
## Corpus Check
- 392 files · ~1,182,404 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 12248 nodes · 65982 edges · 392 communities detected
- Extraction: 21% EXTRACTED · 79% INFERRED · 0% AMBIGUOUS · INFERRED: 52233 edges (avg confidence: 0.5)
...
1. `Sentence` - 3628 edges
2. `GenerationContext` - 1655 edges
3. `CancellationError` - 1329 edges
4. `ProviderError` - 1308 edges
5. `DatabaseIntegrityError` - 1307 edges
6. `RateLimitedError` - 1261 edges
7. `IngestTranscriptCue` - 952 edges
8. `EmbeddingService` - 738 edges
9. `IngestError` - 718 edges
10. `UnsupportedSourceError` - 712 edges
...
Cohesion: 0.02
Nodes (695): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Record one distinct forward navigation without changing watch analytics. (+687 more)

### Community 3 - "Community 3"
Cohesion: 0.08
Nodes (452): cue_text(), filter_by_query(), pick_best_clip(), Bridge: translates clip_engine output into IngestionPipeline._persist_ingest() i, Construct the canonical YouTube source reference used by persistence., Build IngestSegment from a clip dict; text is in-window transcript or clip title, Token-overlap relevance of a clip against a query.      Returns 1.0 when query i, When query is falsy, return clips unchanged.     Otherwise: annotate each clip w (+444 more)

### Community 4 - "Community 4"
Cohesion: 0.29
Nodes (398): AssessmentCancelledError, AssessmentService, EngineError, Base error for the clip engine., GenerationQueueFullError, JobLeaseLostError, Generate a verification code, store its hash, and return the plaintext code., Generate a verification code, store its hash, and return the plaintext code. (+390 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (260): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+252 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (308): Enum, _acquire_selector_slot(), _apply_enrichment(), _AtomicDeclarative, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _boundary_selector_content() (+300 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (210): _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows(), _ensure_learner_progress(), _information_units() (+202 more)
...
Cohesion: 0.01
Nodes (235): ProviderBudgetExceededError, _AssessmentDraft, _BoundaryPlan, _BoundaryRepairCandidate, _BoundaryRepairItem, _BoundaryRepairPlan, _BoundaryTopic, _CardEnrichmentItem (+227 more)
...
Cohesion: 0.02
Nodes (157): _call_telemetry(), _cancel_requested(), count_request_tokens(), _create_client(), _default_gemini3_thinking_level(), _error_code(), _exception_headers(), _field() (+149 more)
...
Cohesion: 0.02
Nodes (132): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+124 more)
...
## Knowledge Gaps
- **3778 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+3773 more)
...
- **Thin community `Community 200`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `Count one pathological long structured request without generating output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `Extract a typed HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `A bounded Gemini FileData Part for one canonical public YouTube video.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `Short-lived direct audio source resolved by yt-dlp.      URL and headers may con`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `Accept only rows with explicit strict energy-threshold evidence.      Measured e`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `Accept strict silence or an explicit, bounded transcript fallback.      Context-`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 244`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 247`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `Extract an HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 254`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 255`** (1 nodes): `A Gemini FileData Part for one canonical public YouTube video URL.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 256`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 258`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 260`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 262`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 264`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 267`** (1 nodes): `Provider metadata for one logical Gemini structured-content call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `A typed Gemini failure carrying telemetry collected before failure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 269`** (1 nodes): `Run ``call`` (returns response text or falsy) with backoff on transient errors.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 270`** (1 nodes): `Extract a typed HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 271`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 272`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 273`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 274`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 275`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 276`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 277`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 278`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 279`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 280`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 281`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 287`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (1 nodes): `Exact cue 0-30 slice from the 129-cue production Supadata artifact.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 348`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 349`** (1 nodes): `Align a uniquely quoted partial edge to explicit lexical onsets.      ``start```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 357`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 358`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 359`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 360`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 361`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 362`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 363`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 364`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 365`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 366`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 367`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 368`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 369`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 372`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 373`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 374`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 375`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 376`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 377`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 378`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 379`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 380`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 381`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 382`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 383`** (1 nodes): `Production regression: ``advanced ATP`` retrieved SAP S/4HANA for a         cell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 384`** (1 nodes): `Text/file materials may have no subject tag. Their sibling concepts         stil`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 385`** (1 nodes): `Old completed inventories are replayed without regeneration, so the         serv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 386`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 387`** (1 nodes): `Return the first letter/digit index, ignoring opening quotes/brackets.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 388`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 389`** (1 nodes): `Reject only question references without context earlier in the same cue.      Ca`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 390`** (1 nodes): `Find opening references without a lexical antecedent in accumulated context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 391`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Sentence` connect `Community 0` to `Community 96`, `Community 65`, `Community 1`, `Community 2`, `Community 99`, `Community 100`, `Community 6`, `Community 104`, `Community 9`, `Community 10`, `Community 14`, `Community 117`, `Community 118`, `Community 121`, `Community 26`, `Community 92`?**
  _High betweenness centrality (0.390) - this node is a cross-community bridge._
- **Why does `GenerationContext` connect `Community 2` to `Community 0`, `Community 65`, `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 33`, `Community 9`, `Community 12`, `Community 14`, `Community 27`, `Community 29`?**
  _High betweenness centrality (0.197) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 4` to `Community 2`, `Community 67`, `Community 40`, `Community 11`, `Community 13`, `Community 21`, `Community 87`, `Community 57`, `Community 58`, `Community 93`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Are the 3622 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 3622 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1636 inferred relationships involving `GenerationContext` (e.g. with `_StrictModel` and `_UncertaintyReason`) actually correct?**
  _`GenerationContext` has 1636 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1326 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 1326 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1294 inferred relationships involving `ProviderError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ProviderError` has 1294 INFERRED edges - model-reasoned connections that need verification._