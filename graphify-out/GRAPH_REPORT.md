...
## Corpus Check
- 392 files · ~1,180,399 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 12107 nodes · 64327 edges · 385 communities detected
- Extraction: 21% EXTRACTED · 79% INFERRED · 0% AMBIGUOUS · INFERRED: 50594 edges (avg confidence: 0.5)
...
1. `Sentence` - 3552 edges
2. `GenerationContext` - 1482 edges
3. `CancellationError` - 1280 edges
4. `DatabaseIntegrityError` - 1260 edges
5. `ProviderError` - 1259 edges
6. `RateLimitedError` - 1214 edges
7. `IngestTranscriptCue` - 911 edges
8. `EmbeddingService` - 707 edges
9. `IngestError` - 687 edges
10. `UnsupportedSourceError` - 681 edges
...
Cohesion: 0.0
Nodes (3131): Run one localized Flash batch and merge only independently valid repairs., Validate one response, salvaging valid boundary candidates independently., Bound selector dispatches across jobs without hiding cancellation., Run one localized Flash batch and merge only independently valid repairs., Run the one aggregate, boundary-only Pro fallback on an existing transcript., Bound selector dispatches across jobs without hiding cancellation., Run one localized Flash batch and merge only independently valid repairs., Remove only the unreliable lowercase signal from auto-caption guards. (+3123 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (798): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+790 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (640): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Record one distinct forward navigation without changing watch analytics. (+632 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (399): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+391 more)

### Community 4 - "Community 4"
Cohesion: 0.08
Nodes (427): cue_text(), filter_by_query(), pick_best_clip(), Bridge: translates clip_engine output into IngestionPipeline._persist_ingest() i, Construct the canonical YouTube source reference used by persistence., Build IngestSegment from a clip dict; text is in-window transcript or clip title, Token-overlap relevance of a clip against a query.      Returns 1.0 when query i, When query is falsy, return clips unchanged.     Otherwise: annotate each clip w (+419 more)

### Community 5 - "Community 5"
Cohesion: 0.29
Nodes (392): AssessmentCancelledError, AssessmentService, EngineError, Base error for the clip engine., GenerationQueueFullError, JobLeaseLostError, Generate a verification code, store its hash, and return the plaintext code., Generate a verification code, store its hash, and return the plaintext code. (+384 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (309): Enum, _acquire_selector_slot(), _apply_enrichment(), _AtomicDeclarative, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _boundary_selector_content() (+301 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (234): ABC, Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed() (+226 more)

### Community 8 - "Community 8"
Cohesion: 0.01
Nodes (238): ProviderBudgetExceededError, _AssessmentDraft, _BoundaryPlan, _BoundaryRepairCandidate, _BoundaryRepairItem, _BoundaryRepairPlan, _BoundaryTopic, _CardEnrichmentItem (+230 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (174): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+166 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (167): _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows(), _ensure_learner_progress(), _information_units() (+159 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (158): _artifact_path(), _chunk_path(), load_artifact(), load_chunk(), On-disk caching for punctuation.  Two levels, both under ``work/<video_id>/``: -, save_artifact(), save_chunk(), transcript_fingerprint() (+150 more)
...
Cohesion: 0.02
Nodes (66): difficulty_matches_knowledge_level(), effective_level_target(), normalize_knowledge_level(), Knowledge-level semantics: level names, difficulty-scale mapping, and the effect, Lowercased/stripped level name; absent -> 'beginner'; unknown -> ValueError., The difficulty the feed should aim at RIGHT NOW for this material., Match selector difficulty bins without overlap at level boundaries., _difficulty() (+58 more)
...
Cohesion: 0.02
Nodes (117): _compact_plan(), _intent_plan(), _proposal(), test_articleless_look_at_visual_noun_remains_visual_dependent(), test_assignment_leadin_is_retained_when_its_cue_contains_grounded_teaching(), test_back_to_navigation_does_not_delete_worked_example_setup(), test_bare_look_at_this_remains_visual_dependent(), test_boundary_quote_reanchoring_never_discards_substantive_context() (+109 more)
...
Cohesion: 0.06
Nodes (69): _discovery(), _FixedSemanticEmbedding, _one_cue_selector_result(), _pipeline(), _pipeline_with_semantic(), _plan(), _quality_clip(), A complete current selector result grounded to one cue. (+61 more)
...
Cohesion: 0.04
Nodes (40): block_emb(), make_sents(), n sentences, `sec` seconds each; a `gap`-second pause BEFORE each index in gap_a, Unit vectors; block j points along axis j%dim → maximal between-block scatter at, _mk_settings(), test_embed_failure_falls_back_to_llm_engine(), test_empty_sentences_video_only(), test_label_failure_keeps_partition() (+32 more)
...
## Knowledge Gaps
- **3816 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+3811 more)
...
- **Thin community `Community 200`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Count one pathological long structured request without generating output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Extract a typed HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `A bounded Gemini FileData Part for one canonical public YouTube video.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 219`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Short-lived direct audio source resolved by yt-dlp.      URL and headers may con`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `Accept only rows with explicit strict energy-threshold evidence.      Measured e`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `Accept strict silence or an explicit, bounded transcript fallback.      Context-`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 240`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Extract an HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 247`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `A Gemini FileData Part for one canonical public YouTube video URL.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 249`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 253`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 255`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 256`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 258`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 259`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 260`** (1 nodes): `Provider metadata for one logical Gemini structured-content call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `A typed Gemini failure carrying telemetry collected before failure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 262`** (1 nodes): `Run ``call`` (returns response text or falsy) with backoff on transient errors.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `Extract a typed HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 264`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 267`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 269`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 271`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 272`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 273`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 274`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 280`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 281`** (1 nodes): `Exact cue 0-30 slice from the 129-cue production Supadata artifact.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 287`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 289`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 290`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 291`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 292`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 294`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 341`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 342`** (1 nodes): `Align a uniquely quoted partial edge to explicit lexical onsets.      ``start```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 343`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 344`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 345`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 346`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 347`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 348`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 349`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 350`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 351`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 352`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 353`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 354`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 355`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 356`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 357`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 358`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 359`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 360`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 361`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 362`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 363`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 364`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 365`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 366`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 367`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 368`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 369`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 370`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 371`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 372`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 373`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 374`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 375`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 376`** (1 nodes): `Production regression: ``advanced ATP`` retrieved SAP S/4HANA for a         cell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 377`** (1 nodes): `Text/file materials may have no subject tag. Their sibling concepts         stil`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 378`** (1 nodes): `Old completed inventories are replayed without regeneration, so the         serv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 379`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 380`** (1 nodes): `Return the first letter/digit index, ignoring opening quotes/brackets.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 381`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 382`** (1 nodes): `Reject only question references without context earlier in the same cue.      Ca`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 383`** (1 nodes): `Find opening references without a lexical antecedent in accumulated context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 384`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Sentence` connect `Community 0` to `Community 96`, `Community 1`, `Community 2`, `Community 99`, `Community 100`, `Community 6`, `Community 8`, `Community 72`, `Community 9`, `Community 11`, `Community 104`, `Community 117`, `Community 118`, `Community 121`, `Community 26`, `Community 92`?**
  _High betweenness centrality (0.393) - this node is a cross-community bridge._
- **Why does `GenerationContext` connect `Community 2` to `Community 32`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 72`, `Community 43`, `Community 20`, `Community 27`, `Community 28`?**
  _High betweenness centrality (0.189) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 5` to `Community 2`, `Community 66`, `Community 40`, `Community 9`, `Community 12`, `Community 13`, `Community 19`, `Community 87`, `Community 58`, `Community 59`, `Community 93`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Are the 3546 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 3546 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1463 inferred relationships involving `GenerationContext` (e.g. with `_StrictModel` and `_UncertaintyReason`) actually correct?**
  _`GenerationContext` has 1463 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1277 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 1277 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1256 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 1256 INFERRED edges - model-reasoned connections that need verification._