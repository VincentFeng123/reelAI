...
## Corpus Check
- 388 files · ~1,056,664 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6557 nodes · 17772 edges · 210 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 6148 edges (avg confidence: 0.5)
...
3. `Unit` - 180 edges
4. `GenerationContext` - 157 edges
5. `CancellationError` - 136 edges
6. `YouTubeService` - 132 edges
7. `DatabaseIntegrityError` - 128 edges
8. `ProviderError` - 124 edges
...
Cohesion: 0.01
Nodes (467): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+459 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (348): AssessmentCancelledError, AssessmentService, _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics. (+340 more)
...
Cohesion: 0.02
Nodes (48): BaseSettings, get_settings(), Settings, _difficulty(), _get_punct_pipeline(), _has_selection_contract(), _importance_ranker_enabled(), _is_short_leaf_topic() (+40 more)
...
Cohesion: 0.03
Nodes (159): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _BoundaryPlan, _BoundaryRepairCandidate (+151 more)
...
Cohesion: 0.03
Nodes (108): cue_text(), filter_by_query(), pick_best_clip(), relevance_score(), synth_adapter_result(), to_cues(), to_metadata(), to_segment() (+100 more)
...
Cohesion: 0.03
Nodes (82): extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), _adapt_query_for_postgres(), _connect_postgres_with_retry(), _database_url(), _db_path() (+74 more)
...
Cohesion: 0.06
Nodes (67): _call_telemetry(), _cancel_requested(), _create_client(), _default_gemini3_thinking_level(), _field(), _finish_reason(), _gemini3_json_schema(), _gemini_status_code() (+59 more)
...
Cohesion: 0.08
Nodes (11): _blend_engine_out(), _build_engine_out(), _clip_side_effect(), _difficulty_engine_out(), _fractional_engine_out(), IngestTopicProgressTests, IngestTopicTests, _no_difficulty_engine_out() (+3 more)
...
Cohesion: 0.08
Nodes (36): fetch_chapters(), main(), make_skeleton(), merge_chapters_into_golden(), Dump a golden *skeleton* from the predicted structure, for a human to correct (s, MERGE {"chapters", "chapters_provenance": "creator"} into <golden_dir>/<video_id, Import creator chapters for each id; report + write nothing when a video has non, Creator chapters via the yt_dlp Python API — metadata only     (``extract_info(d (+28 more)
...
Cohesion: 0.13
Nodes (5): _Resp, test_quota_is_typed_and_not_converted_to_empty_success(), test_rate_limit_respects_bounded_retry_and_records_each_response(), test_search_all_keeps_primary_when_optional_retries_consume_budget(), test_search_all_raises_when_budget_exhausts_before_primary_succeeds()
...
Cohesion: 0.23
Nodes (14): _install_client(), test_async_generated_transcript_job_is_polled_and_cached(), test_async_polling_stops_at_shared_generation_deadline(), test_auto_transcript_preserves_cue_times_language_and_caches(), test_cancelled_transcript_retrieval_never_calls_provider(), test_existing_native_artifact_is_preferred_before_auto_provider_call(), test_expired_generation_deadline_stops_before_provider_call(), test_malformed_nonmonotonic_transcript_is_not_cached() (+6 more)
...
Cohesion: 0.32
Nodes (11): _bootstrap_pool_has_subject_coverage(), _difficulty_bootstrap_query(), discover(), discover_practice_fast(), _load_query_plan(), _long_topic_component_queries(), _niche_bootstrap_backoff_query(), _planned_query_offset() (+3 more)
...
## Knowledge Gaps
- **389 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+384 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 174`** (2 nodes): `ProcessingStepper.tsx`, `ProcessingStepper()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (2 nodes): `ResultsGrid.tsx`, `ResultsGrid()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 176`** (2 nodes): `DownloadAll()`, `DownloadAll.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 177`** (2 nodes): `VolumetricGlowBackground.tsx`, `VolumetricGlowBackground()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 178`** (2 nodes): `useJobStream.ts`, `useJobStream()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 179`** (2 nodes): `test_topics_types.py`, `test_schemas_and_dataclasses_exist()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 180`** (2 nodes): `test_youtube_transport.py`, `test_youtube_impersonation_target_is_supported()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 181`** (2 nodes): `test_provider_cache_db.py`, `test_sqlite_provider_cache_round_trip_and_tombstone_filter()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (2 nodes): `test_singleflight.py`, `test_singleflight_coalesces_identical_concurrent_work()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (2 nodes): `RootLayout()`, `layout.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 184`** (2 nodes): `Loading()`, `loading.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (2 nodes): `FullscreenLoadingScreen()`, `FullscreenLoadingScreen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (2 nodes): `ViewportModalPortal.tsx`, `ViewportModalPortal()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (2 nodes): `youtubeIframeApi.ts`, `loadYouTubeIframeApi()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 188`** (1 nodes): `next-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (1 nodes): `tailwind.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `postcss.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (1 nodes): `next.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (1 nodes): `tailwind.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (1 nodes): `postcss.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (1 nodes): `main.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `SettingsDrawer.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `InputCard.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 207`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `GenerationProgress.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ReelService` connect `Community 3` to `Community 1`, `Community 66`, `Community 97`, `Community 8`, `Community 57`, `Community 93`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 0` to `Community 96`, `Community 1`, `Community 99`, `Community 4`, `Community 5`, `Community 37`, `Community 6`, `Community 105`, `Community 106`, `Community 20`, `Community 122`, `Community 127`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 8`, `Community 162`, `Community 18`, `Community 3`?**
...
- **Are the 139 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 139 INFERRED edges - model-reasoned connections that need verification._