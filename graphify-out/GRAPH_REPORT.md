...
## Corpus Check
- 388 files · ~1,079,185 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6982 nodes · 20368 edges · 205 communities detected
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 8279 edges (avg confidence: 0.5)
...
1. `Sentence` - 331 edges
2. `GenerationContext` - 260 edges
3. `CancellationError` - 240 edges
4. `DatabaseIntegrityError` - 225 edges
5. `ProviderError` - 222 edges
6. `ReelService` - 199 edges
7. `RateLimitedError` - 192 edges
8. `Unit` - 180 edges
9. `IngestTranscriptCue` - 174 edges
10. `EmbeddingService` - 141 edges
...
- `Sentence` --uses--> `AI segment selection — informative, self-contained clips WITH CONTEXT.  The mode`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/select.py
- `Sentence` --uses--> `n sentences, `sec` seconds each; a `gap`-second pause BEFORE each index in gap_a`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/understand/tests/conftest.py
- `Sentence` --uses--> `Unit vectors; block j points along axis j%dim → maximal between-block scatter at`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/understand/tests/conftest.py
- `Sentence` --uses--> `Silence-aware start/end placement (Tasks 5-6). Offline: energy_fn=None → pure ga`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/tests/test_silence_snap.py

## Communities

### Community 0 - "Community 0"
Cohesion: 0.01
Nodes (460): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+452 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (387): Enum, _acquire_selector_slot(), _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts() (+379 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (346): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics., Use a savepoint so multi-row assessment writes cannot be half-persisted., Bridge: translates clip_engine output into IngestionPipeline._persist_ingest() i (+338 more)
...
Cohesion: 0.02
Nodes (181): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+173 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (221): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed(), _bare_video_id() (+213 more)
...
Cohesion: 0.15
Nodes (142): AssessmentCancelledError, AssessmentService, _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows() (+134 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (129): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+121 more)
...
Cohesion: 0.03
Nodes (82): _assessment(), Strict shipping contract for the guarded Gemini educational selector., _run(), _segs(), _selection_task_tail(), test_adjacent_facets_inside_one_coarse_cue_remain_distinct(), test_ambiguous_projected_edge_quote_falls_back_without_rejection(), test_apostrophe_typography_is_grounded_and_source_spelling_is_preserved() (+74 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (108): _adapt_query_for_postgres(), _connect_postgres_with_retry(), _database_url(), _db_path(), ensure_db_initialized(), _ensure_reels_generation_index_postgres(), _ensure_reels_generation_index_sqlite(), execute_modify() (+100 more)
...
Cohesion: 0.04
Nodes (42): extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_ingest_logger(), log_event(), new_trace_id(), Structured logging for the ingestion pipeline.  Every log line emitted from inge (+34 more)
...
Cohesion: 0.04
Nodes (13): _intent_selector(), _live_chain_rule_boundary_segments(), test_boundary_plan_keeps_grounded_unit_when_forward_setup_is_trimmed(), test_candidates_are_validated_independently_without_model_repair(), test_clean_fast_path_never_dispatches_boundary_repair(), test_dirty_edges_use_only_the_one_low_thinking_selector_call(), test_live_chain_rule_candidate_is_rebounded_instead_of_rejected(), test_live_chain_rule_fragment_expands_to_cold_context_and_complete_close() (+5 more)
...
Cohesion: 0.09
Nodes (46): _conn(), _insert_generation_reel(), _patch_request_context(), _set_reel_boundary_state(), _terminal_job_for_generation(), test_authoritative_job_inventory_drops_candidates_absent_from_final_rank(), test_boundary_only_failure_survives_persistence_feed_and_authoritative_replay(), test_completed_job_status_and_replay_drop_currently_invalid_candidate() (+38 more)

### Community 27 - "Community 27"
Cohesion: 0.07
Nodes (44): _clip(), _private_clip(), _report(), _result(), _segments(), test_all_authoritative_pro_calls_use_selected_baseline(), test_boundary_profile_keeps_grounded_clip_with_bad_edge_quote(), test_cancelled_worker_never_publishes_late_boundary_or_done_progress() (+36 more)
...
Cohesion: 0.12
Nodes (33): _assessment(), _convert(), _plan(), _segs(), test_bad_line_indices_are_rejected_instead_of_clamped(), test_boundary_repair_without_explicit_clip_limit_does_not_compare_none(), test_chemistry_fast_path_closes_two_cues_of_missing_setup(), test_clip_limit_uses_overall_quality_before_stable_chronology() (+25 more)
...
Cohesion: 0.06
Nodes (23): bounded_retry_after(), for_mode(), _gemini_cost(), _gemini_physical_attempts(), _gemini_token_rates(), GenerationBudget, Shared provider budgets, retry parsing, and per-generation usage accounting., Atomic, job-wide provider ceilings plus acquisition-pass controls. (+15 more)

### Community 38 - "Community 38"
Cohesion: 0.1
Nodes (37): _intent_plan(), _proposal(), test_articleless_look_at_visual_noun_remains_visual_dependent(), test_bare_look_at_this_remains_visual_dependent(), test_boundary_quote_reanchoring_never_discards_substantive_context(), test_boundary_quote_reanchoring_remains_exact_unique_and_in_range(), test_carolingian_visual_dependent_span_is_rejected(), test_chain_rule_query_keeps_related_prerequisite_and_worked_paraphrase() (+29 more)
...
Cohesion: 0.23
Nodes (26): BaseAdapter, _by_id(), _mk_cand(), role_units(), _same_model(), test_anchor_role_still_reported_in_spec_payload(), test_calculation_answer_satisfies_solution_contract(), test_calculation_as_final_binds_result_not_procedure() (+18 more)
...
Cohesion: 0.13
Nodes (24): cue_text(), filter_by_query(), pick_best_clip(), relevance_score(), synth_adapter_result(), to_cues(), to_metadata(), to_segment() (+16 more)
...
## Knowledge Gaps
- **515 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+510 more)
...
- **Thin community `Community 199`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (1 nodes): `Return the first letter/digit index, ignoring opening quotes/brackets.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Reject only question references without context earlier in the same cue.      Ca`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Find opening references without a lexical antecedent in accumulated context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Sentence` connect `Community 1` to `Community 0`, `Community 2`, `Community 99`, `Community 6`, `Community 8`, `Community 114`, `Community 119`, `Community 23`, `Community 93`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 3` to `Community 64`, `Community 2`, `Community 7`, `Community 46`, `Community 15`, `Community 94`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 2` to `Community 151`, `Community 3`, `Community 63`, `Community 7`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Are the 325 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 325 INFERRED edges - model-reasoned connections that need verification._
- **Are the 242 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 242 INFERRED edges - model-reasoned connections that need verification._
- **Are the 237 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 237 INFERRED edges - model-reasoned connections that need verification._
- **Are the 221 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 221 INFERRED edges - model-reasoned connections that need verification._