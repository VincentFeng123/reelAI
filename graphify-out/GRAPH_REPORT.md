...
## Corpus Check
- 388 files · ~1,051,673 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6499 nodes · 17503 edges · 204 communities detected
- Extraction: 66% EXTRACTED · 34% INFERRED · 0% AMBIGUOUS · INFERRED: 5936 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `Sentence` - 205 edges
2. `ReelService` - 196 edges
3. `Unit` - 180 edges
4. `GenerationContext` - 155 edges
5. `CancellationError` - 134 edges
6. `YouTubeService` - 132 edges
7. `DatabaseIntegrityError` - 124 edges
8. `ProviderError` - 122 edges
...
Cohesion: 0.01
Nodes (464): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+456 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (336): AssessmentCancelledError, AssessmentService, _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics. (+328 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (210): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+202 more)
...
Cohesion: 0.02
Nodes (59): Exception, APIStatusError, BadRequestError, chat(), get_client(), parse_groq_duration(), RateBudget, RateLimitError (+51 more)
...
Cohesion: 0.03
Nodes (157): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _BoundaryPlan, _BoundaryRepairCandidate (+149 more)
...
Cohesion: 0.04
Nodes (79): _anchor(), _run_gate(), _sent(), test_card5_no_introducer_no_rescue_kill_stands(), test_card5_no_suppress_when_subject_absent(), test_card5_seeds_from_introducer_when_not_in_referential(), test_card5_suppress_when_first_sentence_names_anchor_concept(), test_card5_suppress_when_first_sentence_names_topic() (+71 more)

### Community 14 - "Community 14"
Cohesion: 0.04
Nodes (89): _adapt_query_for_postgres(), _connect_postgres_with_retry(), _database_url(), _db_path(), ensure_db_initialized(), _ensure_reels_generation_index_postgres(), _ensure_reels_generation_index_sqlite(), execute_modify() (+81 more)
...
Cohesion: 0.03
Nodes (62): extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_ingest_logger(), log_event(), new_trace_id(), Structured logging for the ingestion pipeline.  Every log line emitted from inge (+54 more)
...
Cohesion: 0.09
Nodes (49): _discovery(), _one_cue_selector_result(), _pipeline(), _pipeline_with_semantic(), _plan(), _quality_clip(), test_acoustic_boundary_plan_fails_closed_for_missing_cue_ids(), test_acoustic_failure_is_diagnostic_only_and_never_persisted_or_emitted() (+41 more)
...
Cohesion: 0.06
Nodes (24): _prepared(), A source/caption edge is not proof of an in-cue semantic handoff., test_background_noise_above_required_threshold_is_unavailable(), test_backward_search_stitches_short_quiet_fragments_across_window_seam(), test_caption_handoff_does_not_scan_an_entire_inter_caption_gap(), test_caption_handoff_never_skips_sound_before_a_late_start_quiet_run(), test_caption_handoff_observation_accepts_only_the_straddling_quiet_run(), test_caption_handoff_rejects_nearby_silence_separated_by_sound() (+16 more)
...
Cohesion: 0.09
Nodes (37): _audio_bitrate(), _audio_entry(), AudioPreparationResult, _canonical_watch_url(), _decode_window(), _EdgeSearchResult, _end_search_windows(), _ffmpeg_headers() (+29 more)
...
Cohesion: 0.16
Nodes (30): _call_v3(), _ConstrainedSchema, _enum_value(), _FakeClient, _FakeModels, _FakeResponse, _HTTPError, _Schema (+22 more)
...
Cohesion: 0.14
Nodes (25): _canonical_feed_url(), canonicalize_youtube_url(), extract_video_id(), normalize_youtube_video_id(), Allowlist channel/playlist paths and discard untrusted query fields., Title/author/duration/thumbnail via yt-dlp (metadata only, no download).     Laz, Return a parsed, credential-free HTTP(S) YouTube URL or ``None``., Resolve a channel/playlist URL to individual YouTube watch URLs (no download). (+17 more)
...
Cohesion: 0.13
Nodes (25): align_edge_anchor(), EdgeAnchor, fetch_json3_words(), _fetch_payload(), _is_original_asr_alias(), _is_untranslated_url(), Json3CaptionTrack, _language_matches() (+17 more)
...
Cohesion: 0.17
Nodes (20): _proposal(), test_carolingian_visual_dependent_span_is_rejected(), test_chain_rule_query_keeps_related_prerequisite_and_worked_paraphrase(), test_comparison_query_keeps_each_substantive_side_as_its_own_facet(), test_context_expands_beyond_eight_cues_and_thirty_seconds(), test_duration_settings_do_not_change_a_complete_clip(), test_each_quality_score_is_an_independent_numeric_hard_gate(), test_exact_topic_gate_generalizes_to_unseen_compound_subjects() (+12 more)
...
Cohesion: 0.31
Nodes (13): _cache_age_seconds(), deterministic_expand(), expand_query(), expand_query_practice_fast(), _expansion_cache_key(), free_expand(), _key(), literal_fallback() (+5 more)
...
## Knowledge Gaps
- **387 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+382 more)
...
- **Why does `ReelService` connect `Community 4` to `Community 1`, `Community 6`, `Community 9`, `Community 51`, `Community 94`, `Community 62`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 0` to `Community 96`, `Community 1`, `Community 5`, `Community 101`, `Community 7`, `Community 8`, `Community 102`, `Community 124`, `Community 19`, `Community 22`, `Community 119`, `Community 92`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 88`, `Community 17`, `Community 4`, `Community 156`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Are the 199 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 199 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 137 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 137 INFERRED edges - model-reasoned connections that need verification._