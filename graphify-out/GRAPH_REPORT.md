...
## Corpus Check
- 392 files · ~1,177,432 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 11893 nodes · 58857 edges · 367 communities detected
- Extraction: 23% EXTRACTED · 77% INFERRED · 0% AMBIGUOUS · INFERRED: 45169 edges (avg confidence: 0.5)
...
1. `Sentence` - 3470 edges
2. `GenerationContext` - 1242 edges
3. `CancellationError` - 1201 edges
4. `ProviderError` - 1180 edges
5. `DatabaseIntegrityError` - 1178 edges
6. `RateLimitedError` - 1135 edges
7. `IngestTranscriptCue` - 883 edges
8. `EmbeddingService` - 645 edges
9. `IngestError` - 625 edges
10. `UnsupportedSourceError` - 619 edges
...
- `Render only the neighboring cue windows needed to repair dirty edges.` --uses--> `Sentence`  [INFERRED]
  backend/pipeline/gemini_segment.py → backend/pipeline/sentences.py
- `Sentence` --uses--> `Remove claims whose only transcript support was cut by range repair.`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/gemini_segment.py
- `Sentence` --uses--> `Ground one semantic edge without inventing a timestamp.      The edge occurrence`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/gemini_segment.py
...
Cohesion: 0.02
Nodes (306): Enum, _acquire_selector_slot(), _apply_enrichment(), _AtomicDeclarative, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _boundary_selector_content() (+298 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (163): SearchError, _estimate_tokens(), _groq_json_schema(), _json_instruction(), llm_json(), llm_json_result(), One provider-abstracted structured-JSON LLM call.  Every structure-first pass (c, A strict 'return only JSON of this shape' instruction for the Groq json_object p (+155 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (190): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+182 more)
...
Cohesion: 0.01
Nodes (229): ProviderBudgetExceededError, _AssessmentDraft, _BoundaryPlan, _BoundaryRepairCandidate, _BoundaryRepairItem, _BoundaryRepairPlan, _BoundaryTopic, _CardEnrichmentItem (+221 more)
...
Cohesion: 0.02
Nodes (177): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+169 more)

### Community 13 - "Community 13"
Cohesion: 0.03
Nodes (154): _call_telemetry(), _cancel_requested(), count_request_tokens(), _create_client(), _default_gemini3_thinking_level(), _error_code(), _exception_headers(), _field() (+146 more)
...
Cohesion: 0.03
Nodes (90): _assessment(), _boundary_topic(), Strict shipping contract for the guarded Gemini educational selector., _run(), _segs(), _selection_task_tail(), test_adjacent_facets_inside_one_coarse_cue_remain_distinct(), test_ambiguous_projected_edge_quote_falls_back_without_rejection() (+82 more)
...
Cohesion: 0.03
Nodes (92): _atomic_write(), _available_questions(), _cadence_session_id(), _cadence_target(), _check_cancelled(), _completed_rows(), _ensure_learner_progress(), _information_units() (+84 more)
...
Cohesion: 0.15
Nodes (4): _mk_candidate(), test_grounding_score_not_inflated_on_error(), test_repair_returns_candidate_on_error_without_burning_budget(), test_snap_flags_and_warns_unjudged()
...
## Knowledge Gaps
- **3703 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+3698 more)
...
- **Thin community `Community 200`** (1 nodes): `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Short-lived direct audio source resolved by yt-dlp.      URL and headers may con`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Accept only rows with explicit strict energy-threshold evidence.      Measured e`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Accept strict silence or an explicit, bounded transcript fallback.      Context-`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 220`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Extract an HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 227`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `A Gemini FileData Part for one canonical public YouTube video URL.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 235`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `When discover() returns a truthy "warning" (e.g. out-of-credits) and no videos,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Provider metadata for one logical Gemini structured-content call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `A typed Gemini failure carrying telemetry collected before failure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `Run ``call`` (returns response text or falsy) with backoff on transient errors.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `Extract a typed HTTP status without relying on provider error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 250`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): `Invalidate cache entries whenever the active prompt or validators change.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 253`** (1 nodes): `Keep evaluation side effects and source-less releases out of the cache.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (1 nodes): `Hash every input that can change the validated public clip list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 260`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `Exact cue 0-30 slice from the 129-cue production Supadata artifact.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 262`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 267`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 274`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 275`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 276`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 277`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 281`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 282`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 283`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 284`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 285`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 286`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 287`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 289`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 290`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 291`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 292`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 294`** (1 nodes): `Return nearest-first, non-overlapping windows extending forward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 295`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `Classify resolver failures without retaining sensitive command output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `Kill yt-dlp and any Deno child without an unbounded pipe wait.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `Resolve bestaudio without downloading it, suitable for parallel prefetch.      C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 300`** (1 nodes): `Return nearest-first, non-overlapping windows extending backward.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 321`** (1 nodes): `Verify and adjust one clip to measured quiet edge intervals.      ``start_sec```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 322`** (1 nodes): `Align a uniquely quoted partial edge to explicit lexical onsets.      ``start```
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 337`** (1 nodes): `A source/caption edge is not proof of an in-cue semantic handoff.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 338`** (1 nodes): `Return a bounded provider Retry-After delay without retaining error prose.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 339`** (1 nodes): `Wait for a retry while polling the generation cancellation callback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 340`** (1 nodes): `Return the JSON-Schema subset accepted by Gemini 3 GenerateContent.      Gemini`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 341`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 342`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 343`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 344`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 345`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 346`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 347`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 348`** (1 nodes): `Run one Gemini 3 structured call with bounded transport behavior.      SDK retri`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 349`** (1 nodes): `Return a JSON string conforming to `schema` (a Pydantic model class).      ``max`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 350`** (1 nodes): `A Gemini image Part from raw JPEG bytes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 351`** (1 nodes): `Map a config string ('low'|'medium'|'high') → a ``types.MediaResolution`` enum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 352`** (1 nodes): `A Gemini video Part from raw mp4 bytes, sent INLINE (use when the request is sma`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 353`** (1 nodes): `Return a JSON string conforming to ``schema`` from a VIDEO+text prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 354`** (1 nodes): `Return a JSON string conforming to ``schema`` from a multimodal prompt.      ``p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 355`** (1 nodes): `Two relevance-surviving clips from one video's transcript. Each clip's     windo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 356`** (1 nodes): `One 300s whole-topic clip — the kind the practice (Gemini) engine cuts for     a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 357`** (1 nodes): `Context managers patching discover + run.clip at the pipeline aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 358`** (1 nodes): `Production regression: ``advanced ATP`` retrieved SAP S/4HANA for a         cell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 359`** (1 nodes): `Text/file materials may have no subject tag. Their sibling concepts         stil`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 360`** (1 nodes): `Old completed inventories are replayed without regeneration, so the         serv`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 361`** (1 nodes): `Serve-time level scoring: matched clips first, off-level kept at the     back, a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 362`** (1 nodes): `Return the first letter/digit index, ignoring opening quotes/brackets.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 363`** (1 nodes): `A leading marker is fine when the sentence stands on its own as new framing/a qu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 364`** (1 nodes): `Reject only question references without context earlier in the same cue.      Ca`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 365`** (1 nodes): `Find opening references without a lexical antecedent in accumulated context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 366`** (1 nodes): `True when this sentence, as a clip's first line, drops the viewer mid-thought.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Sentence` connect `Community 0` to `Community 1`, `Community 2`, `Community 97`, `Community 100`, `Community 5`, `Community 101`, `Community 71`, `Community 7`, `Community 9`, `Community 10`, `Community 105`, `Community 118`, `Community 119`, `Community 122`, `Community 28`, `Community 93`?**
...
- **Why does `ProviderBudgetExceededError` connect `Community 9` to `Community 2`, `Community 4`, `Community 5`, `Community 6`, `Community 71`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Are the 3464 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 3464 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1223 inferred relationships involving `GenerationContext` (e.g. with `_HTTPStatusError` and `_ModelSequence`) actually correct?**
  _`GenerationContext` has 1223 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1198 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 1198 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1166 inferred relationships involving `ProviderError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ProviderError` has 1166 INFERRED edges - model-reasoned connections that need verification._