...
2. `GenerationContext` - 1156 edges
3. `CancellationError` - 1117 edges
4. `DatabaseIntegrityError` - 1098 edges
5. `ProviderError` - 1095 edges
6. `RateLimitedError` - 1055 edges
7. `IngestTranscriptCue` - 859 edges
8. `EmbeddingService` - 585 edges
9. `IngestError` - 565 edges
10. `UnsupportedSourceError` - 559 edges
...
- `Sentence` --uses--> `Render only the neighboring cue windows needed to repair dirty edges.`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/gemini_segment.py
- `Sentence` --uses--> `Detect a caption fragment that contains only the tail of a question.`  [INFERRED]
  backend/pipeline/sentences.py → backend/pipeline/gemini_segment.py
...
Cohesion: 0.03
Nodes (152): _call_telemetry(), _cancel_requested(), _create_client(), _default_gemini3_thinking_level(), _error_code(), _exception_headers(), _field(), _finish_reason() (+144 more)
...
- **Are the 1137 inferred relationships involving `GenerationContext` (e.g. with `_HTTPStatusError` and `_ModelSequence`) actually correct?**
  _`GenerationContext` has 1137 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1114 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 1114 INFERRED edges - model-reasoned connections that need verification._
- **Are the 1094 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 1094 INFERRED edges - model-reasoned connections that need verification._