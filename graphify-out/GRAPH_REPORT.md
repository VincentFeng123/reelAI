...
## Corpus Check
- 387 files · ~1,046,383 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6446 nodes · 17397 edges · 202 communities detected
- Extraction: 66% EXTRACTED · 34% INFERRED · 0% AMBIGUOUS · INFERRED: 5962 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `Sentence` - 203 edges
2. `ReelService` - 196 edges
3. `Unit` - 180 edges
4. `GenerationContext` - 156 edges
5. `CancellationError` - 135 edges
6. `YouTubeService` - 132 edges
7. `DatabaseIntegrityError` - 125 edges
8. `ProviderError` - 123 edges
...
Cohesion: 0.01
Nodes (483): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+475 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (308): AssessmentCancelledError, AssessmentService, _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Promote one named level after sustained, broad mastery evidence.          Manual, Validate and persist one private answer-bearing question for a reel., Choose a stable 2-5 reel cadence, then adapt it to current evidence., Record one distinct forward navigation without changing watch analytics. (+300 more)
...
Cohesion: 0.02
Nodes (46): _ChainBufferingEmitter, _difficulty(), _get_punct_pipeline(), _has_selection_contract(), _importance_ranker_enabled(), _is_short_leaf_topic(), ReelService, _selection_contract_sort_key() (+38 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (214): _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _auto_verify_community_account_if_allowed(), _bare_video_id(), _build_allowed_origins(), _build_generation_head_id(), can_generate_reels() (+206 more)
...
Cohesion: 0.03
Nodes (153): _apply_enrichment(), _Assessment, _AssessmentDraft, _authoritative_pro(), _boundary_prompts(), _boundary_repair_prompts(), _BoundaryPlan, _BoundaryRepairCandidate (+145 more)
...
Cohesion: 0.03
Nodes (111): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+103 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (75): _assessment(), Strict shipping contract for the guarded Gemini educational selector., _run(), _segs(), _selection_task_tail(), test_adjacent_facets_inside_one_coarse_cue_remain_distinct(), test_ambiguous_projected_edge_quote_is_rejected(), test_boundary_schema_defaults_compact_nonessential_fields() (+67 more)
...
Cohesion: 0.03
Nodes (110): best_match(), find_human_clip(), gold_chapters(), human_block(), iou(), merge_human_into_golden(), merge_human_labels(), _norm_human_label() (+102 more)
...
Cohesion: 0.1
Nodes (45): _discovery(), _one_cue_selector_result(), _pipeline(), _pipeline_with_semantic(), _plan(), _quality_clip(), test_acoustic_failure_is_stored_but_never_emitted(), test_acoustic_failures_fail_closed_without_pro_repair() (+37 more)
...
Cohesion: 0.12
Nodes (38): _conn(), _insert_generation_reel(), _patch_request_context(), _set_reel_boundary_state(), _terminal_job_for_generation(), test_authoritative_job_inventory_drops_candidates_absent_from_final_rank(), test_completed_job_status_and_replay_drop_currently_invalid_candidate(), test_cross_level_reservoir_rejects_unverified_or_implicit_surface_rows() (+30 more)
...
Cohesion: 0.08
Nodes (23): _prepared(), A source/caption edge is not proof of an in-cue semantic handoff., test_background_noise_above_required_threshold_is_unavailable(), test_backward_search_stitches_short_quiet_fragments_across_window_seam(), test_caption_handoff_does_not_scan_an_entire_inter_caption_gap(), test_caption_handoff_never_skips_sound_before_a_late_start_quiet_run(), test_caption_handoff_observation_accepts_only_the_straddling_quiet_run(), test_caption_handoff_rejects_nearby_silence_separated_by_sound() (+15 more)
...
Cohesion: 0.17
Nodes (27): _call_v3(), _ConstrainedSchema, _enum_value(), _FakeClient, _FakeModels, _FakeResponse, _HTTPError, _Schema (+19 more)
...
Cohesion: 0.14
Nodes (22): _acoustic_range_is_safe(), _cli_main(), _direct_adapter_duration_sec(), _discover(), _exact_topic_corroboration_scores(), _exact_topic_corroboration_terms(), _grounded_topic_evidence_quote(), _is_valid_timestamped_supadata_transcript() (+14 more)
...
Cohesion: 0.14
Nodes (23): align_edge_anchor(), EdgeAnchor, fetch_json3_words(), _fetch_payload(), _is_untranslated_url(), Json3CaptionTrack, _language_matches(), LexicalWord (+15 more)

### Community 60 - "Community 60"
Cohesion: 0.16
Nodes (20): _build_cache_key(), _build_cerebras_client(), _build_gemini_module(), _build_groq_client(), _build_ollama_client(), _cerebras_chat(), chat_completion(), _collect_gemini_api_keys() (+12 more)
...
Cohesion: 0.16
Nodes (5): DirectAdapterMediaTailTests, _FixedSemanticEmbedding, _media_tail_engine_out(), Regression coverage for quiet handoffs beyond the final caption timestamp., _SemanticVector
...
Cohesion: 0.18
Nodes (17): _age_seconds(), cache_enabled(), _canonical_json(), load_segment_result(), _objective_tokens(), Persistent cache for fully validated Gemini segmentation results., Keep evaluation side effects and source-less releases out of the cache., Hash every input that can change the validated public clip list. (+9 more)
...
Cohesion: 0.21
Nodes (16): _proposal(), test_carolingian_visual_dependent_span_is_rejected(), test_chain_rule_query_keeps_related_prerequisite_and_worked_paraphrase(), test_comparison_query_keeps_each_substantive_side_as_its_own_facet(), test_context_expands_beyond_eight_cues_and_thirty_seconds(), test_duration_settings_do_not_change_a_complete_clip(), test_each_quality_score_is_an_independent_numeric_hard_gate(), test_exact_topic_gate_generalizes_to_unseen_compound_subjects() (+8 more)
...
Cohesion: 0.17
Nodes (14): APIStatusError, BadRequestError, chat(), get_client(), parse_groq_duration(), RateBudget, RateLimitError, Groq SDK wrapper: transcription + chat, with a proactive token-budget limiter an (+6 more)
...
Cohesion: 0.18
Nodes (6): test_alignment_tolerates_provider_drift_away_from_verified_edge(), test_end_anchor_is_first_excluded_suffix_onset(), test_fetches_one_url_with_supplied_transport_bounds(), test_json3_parser_uses_only_explicit_offsets_and_proven_first_word_zero(), test_start_anchor_uses_unique_unicode_normalized_quote_and_prefix(), _word()
...
## Knowledge Gaps
- **386 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+381 more)
...
- **Why does `ReelService` connect `Community 3` to `Community 1`, `Community 7`, `Community 9`, `Community 51`, `Community 90`, `Community 61`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 0` to `Community 96`, `Community 1`, `Community 99`, `Community 100`, `Community 5`, `Community 6`, `Community 8`, `Community 20`, `Community 116`, `Community 121`, `Community 94`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `IngestTranscriptCue` connect `Community 1` to `Community 88`, `Community 154`, `Community 3`, `Community 23`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Are the 197 inferred relationships involving `Sentence` (e.g. with `Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar` and `Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet`) actually correct?**
  _`Sentence` has 197 INFERRED edges - model-reasoned connections that need verification._
- **Are the 79 inferred relationships involving `ReelService` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`ReelService` has 79 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 138 inferred relationships involving `GenerationContext` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`GenerationContext` has 138 INFERRED edges - model-reasoned connections that need verification._