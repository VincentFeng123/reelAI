...
## Corpus Check
- 407 files · ~1,086,217 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8549 nodes · 30180 edges · 201 communities detected
- Extraction: 52% EXTRACTED · 48% INFERRED · 0% AMBIGUOUS · INFERRED: 14448 edges (avg confidence: 0.5)
...
1. `GenerationContext` - 717 edges
2. `Sentence` - 532 edges
3. `CancellationError` - 292 edges
4. `DatabaseIntegrityError` - 286 edges
5. `ProviderError` - 266 edges
6. `ReelService` - 252 edges
7. `RateLimitedError` - 229 edges
8. `EmbeddingService` - 201 edges
9. `IngestError` - 181 edges
10. `Unit` - 180 edges
...
Cohesion: 0.01
Nodes (590): _arc_verify_enabled(), ArcCandidate, ArcCheckLLM, ArcVerifyLLM, _concepts(), detect_arcs(), _pair_practice_prompts(), Deterministic instructional-arc detection (Wave 2 P3a).  Scans the time-ordered (+582 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (647): Enum, _acquire_selector_slot(), _apply_enrichment(), _Assessment, _AssessmentDraft, _AtomicDeclarative, _audit_pro_boundaries(), _authoritative_pro() (+639 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (375): _BackfillPlan, Adaptive recall-check persistence, readiness, and session lifecycle., Prepare one private recall question for every reel in a released batch., Each organizer checkpoint is one immediate reel-grounded question., Promote one named level after sustained, broad mastery evidence.          Manual, Build an immediate transcript-only recall question without provider I/O., Validate and persist one private answer-bearing question for a reel., Resolve reels against the newest learner-owned released lesson plans.          E (+367 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (283): _await_with_probe(), is_cancelled(), raise_if_cancelled(), Small async-to-sync bridge for actively cancellable provider requests.  The publ, Run an async request from synchronous pipeline code.      Normal generation work, run_cancellable(), sleep_with_probe(), wait_with_probe() (+275 more)
...
Cohesion: 0.01
Nodes (72): MediumRegressionTests, Niche 4C: New static topics should be in STATIC_TOPIC_SUBTOPICS., Bug 1C: _bootstrap_pool_is_weak should not check has_transcript (never set on ca, Bug 1E: Empty search results should only be cached for 15 minutes, not 2 hours., Quality 5A: Known educational channels should get 'known_educational' tier., Niche 4A: Datamuse should be called for opaque topics, with results filtered., Niche 4C: New static topics should be in STATIC_TOPIC_SUBTOPICS., _validated_query_plan() (+64 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (198): _energy_min_snap(), _ensure_audio(), _gap_after(), _gap_before(), _pick_end(), _pick_start(), Precise boundary refinement with targeted Whisper.  Supadata gives fast but coar, Absolute time of the lowest-RMS ``frame_ms`` frame within ``[a, b]`` — the quiet (+190 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (245): Email service using Resend for transactional email delivery., Send a welcome email to a newly registered user.      Errors are caught and logg, send_welcome_email(), _activate_generation(), _adaptive_min_relevance_floor(), answer_assessment(), _apply_generation_lesson_order(), _authoritative_release_reel_ids() (+237 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (53): difficulty_matches_knowledge_level(), effective_level_target(), normalize_knowledge_level(), Knowledge-level semantics: level names, difficulty-scale mapping, and the effect, Lowercased/stripped level name; absent -> 'beginner'; unknown -> ValueError., The difficulty the feed should aim at RIGHT NOW for this material., Match selector difficulty bins without overlap at level boundaries., _difficulty() (+45 more)

### Community 9 - "Community 9"
Cohesion: 0.31
Nodes (184): AssessmentCancelledError, AssessmentService, BaseModel, DailySearchLimitReached, BillingAccountNotFoundError, BillingConfigurationError, BillingVerificationError, DuplicateSubscriptionError (+176 more)
...
Cohesion: 0.02
Nodes (150): ABC, BaseSettings, extract_concepts(), _extract_concepts_via_llm(), Extract higher-quality concepts via Gemini (falling back to Groq)., _summary_for_terms(), get_settings(), Settings (+142 more)
...
Cohesion: 0.04
Nodes (107): _ambiguous_gemini_edge_engine_out(), _discovery(), _FixedSemanticEmbedding, _groq_edge_words(), _one_cue_selector_result(), _pipeline(), _pipeline_with_semantic(), _plan() (+99 more)
...
Cohesion: 0.06
Nodes (82): _append_authoritative_release(), _conn(), _insert_generation_reel(), _patch_released_ranked_feed(), _patch_request_context(), _rank_released_feed(), _released_feed_reel(), _released_generation_chain() (+74 more)
...
Cohesion: 0.05
Nodes (76): _acquire_quota_lock(), _attach_reservation_job(), attach_reservation_to_job(), billing_enforcement_enabled(), billing_entitlement_environment(), billing_status(), _day_and_reset(), effective_plan() (+68 more)
...
Cohesion: 0.05
Nodes (50): fetch_chapters(), main(), make_skeleton(), merge_chapters_into_golden(), Dump a golden *skeleton* from the predicted structure, for a human to correct (s, MERGE {"chapters", "chapters_provenance": "creator"} into <golden_dir>/<video_id, Import creator chapters for each id; report + write nothing when a video has non, Creator chapters via the yt_dlp Python API — metadata only     (``extract_info(d (+42 more)
...
Cohesion: 0.07
Nodes (45): _cache_age_seconds(), _contains_ordered_topic_terms(), _contains_token_phrase(), _covering_focus_indices(), deterministic_expand(), expand_query(), expand_query_practice_fast(), _expansion_cache_key() (+37 more)
...
## Knowledge Gaps
- **424 isolated node(s):** `Submit one autocrop job and return the URL of the rendered short.`, `MuAPI result shapes vary by endpoint — try common keys.`, `Find the most viral-worthy highlights in a transcript.  Logic ported from ViralV`, `Default LLM backend: MuAPI gpt-5-mini.`, `gpt-5-4 sometimes wraps JSON in markdown fences — strip and parse.` (+419 more)
...
- **Thin community `Community 174`** (2 nodes): `FullscreenLoadingScreen()`, `FullscreenLoadingScreen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (2 nodes): `formatAccuracy()`, `RecallCheck.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 176`** (2 nodes): `formatResetTime()`, `BillingPlanUsageCard.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `GenerationContext` connect `Community 1` to `Community 2`, `Community 3`, `Community 36`, `Community 34`, `Community 4`, `Community 8`, `Community 9`, `Community 11`, `Community 14`, `Community 110`, `Community 25`, `Community 28`?**
  _High betweenness centrality (0.189) - this node is a cross-community bridge._
- **Why does `Sentence` connect `Community 1` to `Community 32`, `Community 0`, `Community 2`, `Community 96`, `Community 100`, `Community 97`, `Community 6`, `Community 10`, `Community 114`, `Community 119`, `Community 89`, `Community 27`, `Community 93`?**
  _High betweenness centrality (0.102) - this node is a cross-community bridge._
- **Why does `ReelService` connect `Community 8` to `Community 1`, `Community 2`, `Community 5`, `Community 9`, `Community 42`, `Community 61`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Are the 698 inferred relationships involving `GenerationContext` (e.g. with `_StrictModel` and `_UncertaintyReason`) actually correct?**
  _`GenerationContext` has 698 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 289 inferred relationships involving `CancellationError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`CancellationError` has 289 INFERRED edges - model-reasoned connections that need verification._
- **Are the 282 inferred relationships involving `DatabaseIntegrityError` (e.g. with `Core rate limit check against a pre-built key. Raises HTTPException on breach.` and `Preferred rate-limit bucket key for a request. Uses the owner key hash when`) actually correct?**
  _`DatabaseIntegrityError` has 282 INFERRED edges - model-reasoned connections that need verification._